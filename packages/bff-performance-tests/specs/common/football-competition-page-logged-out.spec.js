...
describe("Competition page logged out", () => {
  let networkRecording;

  beforeAll(
    async function () {
      const competitionData = await weaver.fetchSingleCompetitionData({
        productTypes: ["SPORTSBOOK"],
        selectBy: "RANK",
      });

      this.journeyName = "football-competition-page-logged-out";
      networkRecording = this.networkRecording;
      await networkRecording.recordUrl(`football/_/c-${competitionData.competitionId}?throttlesOff=PRELOAD_CATALOG`);

const { FooterPO, GenericPagePO } = require("@ppb/tbd-tests-web/page-objects");
const { scrollDown } = require("../../utils/scroll");

const genericPagePO = new GenericPagePO();
const footerPO = new FooterPO().element;

describe("Casino home page logged out", () => {
  let networkRecording;

  beforeAll(
    async function () {
      this.journeyName = "casino-home-page-logged-out";
      networkRecording = this.networkRecording;
      await networkRecording.recordUrl("casino/gm-1?throttlesOff=PRELOAD_CATALOG");

...
const Weaver = require("../../../../tools/utils/weaver");

const weaver = new Weaver();
const genericPagePO = new GenericPagePO();
const footerPO = new FooterPO().element;

describe("Horse Race page logged out", () => {
  let networkRecording;

  beforeAll(
    async function () {
      const races = await weaver.fetchWeaverRaces({
      ...
      // By Rank pulls the latest race trying to use the 4th race to make sure is not closed
      const raceIndex = races.results.length < 4 ? 0 : 3;
      const raceData = races.results[raceIndex];

      this.journeyName = "horse-racing-race-page-logged-out";
      networkRecording = this.networkRecording;
      await networkRecording.recordUrl(`horse-racing/_/r-7%7C${raceData.raceId}?throttlesOff=PRELOAD_CATALOG`);

...
describe("StatsPlayersSeasonStats.graphql", () => {
  ...
  describe("when there is already data in the cache", () => {
    it("should return the data from the cache", async () => {
      ...
      expect(await getStatsPlayersSeasonStats("ppb:tbd:card:stats:playersSeasonStats:1")).toBe("fragmentValue");
    });
  });
  describe("when there is not data in the cache", () => {
    it("should return the data from the cache if data or Cards are null", async () => {
      ...
      expect(await getStatsPlayersSeasonStats("ppb:tbd:card:stats:playersSeasonStats:1")).toBe(undefined);
    });

    it("should return the data from the query", async () => {
      getApolloClient().cache.readFragment.mockReturnValueOnce(undefined);
      fetchApolloQuery.mockReturnValueOnce({
        Cards: [{ urn: "ppb:tbd:card:stats:playersSeasonStats:1", fixture: "fixture" }],
      });

      expect(await getStatsPlayersSeasonStats("ppb:tbd:card:stats:playersSeasonStats:1")).toEqual({
        urn: "ppb:tbd:card:stats:playersSeasonStats:1",
        fixture: "fixture",
      });
    });
  });

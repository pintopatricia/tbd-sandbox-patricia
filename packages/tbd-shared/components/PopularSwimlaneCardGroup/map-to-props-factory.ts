...
import type { PopularSwimlaneCardGroups } from "@ppb/tbd-store/state/layout/cardgroups/CardGroup.types";
...
export const makeMapStateToProps: MapStateToPropsFactory<StateProps, ContainerProps, ApplicationState> = () => {
  const getSwimlaneCardGroupByURN = createCardGroupByURNSelector<PopularSwimlaneCardGroups, URN>();

  return function mapStateToProps(state: ApplicationState, { urn: cardgroupURN }: ContainerProps): StateProps {
    const cardgroup = getSwimlaneCardGroupByURN(state.layouts.cardgroups.popularswimlanecardgroups, cardgroupURN);

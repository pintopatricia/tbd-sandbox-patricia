...
import { createRugbyLeagueFixtureViewModel } from "../../view-model-factories/rugby-league-fixture";
...
export const makeMapStateToProps: MapStateToPropsFactory<StateProps, ContainerProps, ApplicationState> = () => {
  ...
  return function mapStateToProps(
    ...
    const rugbyLeagueFixture = getRugbyLeagueFixtureByURN(state.entities.rugbyleaguefixtures, urn);

    if (!rugbyLeagueFixture) return {};

    const { halfTimeScore, score, opponentsNames, isAmericanFormat } = rugbyLeagueFixture;
    ...
    const rugbyLeagueScoreboard = getRugbyLeagueFixtureViewModel({
      userDetails,
      sportEvent,
      score,
      isAmericanFormat,
    ...
    if (rugbyLeagueScoreboard) {
      ({ date, dateTime, time, scoreData } = rugbyLeagueScoreboard);
    }

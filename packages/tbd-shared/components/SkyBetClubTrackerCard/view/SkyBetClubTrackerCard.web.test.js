...
import SkyBetClubTrackerCard from "./SkyBetClubTrackerCard.web";
import useSkyBetClubTrackerVMMockFn from "../viewmodel/SkyBetClubTracker.viewmodel";

jest.mock("./snowflakes/SkyBetClubTracker/SkyBetClubTracker.web", () => ({
  SkyBetClubTracker: jest.fn(() => <div data-testid="sky-bet-club-tracker-mock" />),
}));
...
jest.mock("../viewmodel/SkyBetClubTracker.viewmodel", () => ({
  ...
  __esModule: true,
  default: jest.fn(() => useSkyBetClubTrackerVMMock),
}));

jest.mock("../../../config/endpoints", () => ({
  getEndpoint: jest.fn(() => "skybetClubURL"),
}));
...
describe("SkyBetClubTrackerCard component", () => {
  ...
  describe("when homePageUrl is defined and the primary button is tapped", () => {
    ...
    it("should call navigate action with the correct parameters", () => {
      ...
      expect(window.location.href).toBe(useSkyBetClubTrackerVMMock.vm.data.homePageUrl);
    });
  });
});

...
import { EmptyState, InfoLabel, MarketPromo } from "@ppb/the-wall-native";
...
import StatsPlayersInPlayCard from "./StatsPlayersInPlayCard.native";
...
jest.mock("../../StatsContentCardPlaceholder/StatsContentCardPlaceholder.native", () =>
  jest.fn(() => <stats-content-card-placeholder-mock />),
);

jest.mock("../viewmodel/StatsPlayersInPlayCard.viewmodel", () => jest.fn());

jest.mock("./snowflakes/StatsTable/StatsTable.native", () => jest.fn(() => <stats-table-mock />));
...
describe("StatsPlayersInPlayCard.native", () => {
  ...
  describe("when `useStatsPlayersInPlayCardVM` returns no data", () => {
    describe("and request is still not called", () => {
      ...
      beforeEach(() => {
        useStatsPlayersInPlayCardVM.mockReturnValueOnce(STATS_PLAYERS_IN_PLAY_CARD_VM_NO_DATA_NOT_CALLED);
        ...
        describe("onTermsClick callback", () => {
          it("should call navigate with the correct props", () => {
            ...
            expect(navigate).toHaveBeenCalledWith({
              ...
              viewUrl: STATS_PLAYERS_IN_PLAY_CARD_VM_NO_DATA_CALLED.vm.data.termsAndConditionsURL,

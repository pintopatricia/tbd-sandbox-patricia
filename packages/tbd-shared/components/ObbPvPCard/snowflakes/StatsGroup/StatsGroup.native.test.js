...
import { Placeholder, ProgressBar } from "@ppb/the-wall-native";
import { ProgressBarVariant } from "@ppb/the-wall-common/types";
import { StatsGroup } from "./StatsGroup.native";

import { MAIN_LABEL, SECONDARY_LABEL, STAT_LABEL } from "./StatsGroup.native.selectors";
...
describe("StatsGroup", () => {
  ...
  it("should render with default props", () => {
    ...
    expect(getByTestId(MAIN_LABEL)).toHaveTextContent(defaultProps.label);
  ...
  describe("when disabled is `true`", () => {
    it("should render titles as `disabled`", () => {
      const { getByTestId } = renderComponent({ ...defaultProps, disabled: true });

      expect(getByTestId(MAIN_LABEL)).toHaveStyle(styles.disabled);
      expect(getByTestId(SECONDARY_LABEL)).toHaveStyle(styles.disabled);
    });
  });
  ...
  describe("with left and right players data", () => {
    it("should render labels as `placeholder`", () => {
      ...
      const labels = queryAllByTestId(STAT_LABEL);

      expect(labels[0]).toHaveTextContent("0.23");
      expect(labels[1]).toHaveTextContent("1.46");
    });
  });
});

...
import { CastBetsCard } from "./CastBetsCard.native";

import ConnectedCastBet from "../CastBet";
import { CastBet } from "../CastBet/CastBet.native";

jest.fn("@ppb/the-wall-native/helpers/test-props", () => ({
  getTestProps: jest.fn(() => ({ testID: "testID" })),
}));

jest.mock("../CastBet", () => jest.fn(({ props }) => <connected-cast-bet-mock {...props} />));

jest.mock("../CastBet/CastBet.native", () => jest.fn(({ props }) => <cast-bet-mock {...props} />));

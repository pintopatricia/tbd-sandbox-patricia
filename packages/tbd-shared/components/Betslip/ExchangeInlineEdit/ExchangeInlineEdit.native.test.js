...
import { ExchangeInlineEdit } from "./ExchangeInlineEdit.native";
...
jest.mock("./snowflakes/ExchangeInlineEditPanel/ExchangeInlineEditPanel.native", () => ({
...
describe("ExchangeInlineEdit", () => {
      ...
      expect(ExchangeInlineEditPanel).toHaveBeenCalledWith(
        {
          ...
          persistenceOptions: [{ id: "LAPSE", text: "CancelBet" }],
  ...
  it("should provide a noop onQuickStakeAdd handler", () => {
    renderConnectedExchangeInlineEdit({
      side: ExchangeSide.LAY,
      title: "Lay Bet",
      currencySymbol: "€",
    ...
    const { onQuickStakeAdd } = ExchangeInlineEditPanel.mock.calls[0][0];

    expect(onQuickStakeAdd).toEqual(expect.any(Function));
    expect(() => onQuickStakeAdd()).not.toThrow();
  });
  ...
  describe("when performing interactions from ExchangeInlineEditPanel", () => {
    ...
    describe("when blurring price", () => {
      ...
      it("should not dispatch when there is no price error", () => {
        const dispatchEditPriceInputBlurSpy = jest.fn();
        ...
        expect(dispatchEditPriceInputBlurSpy).not.toHaveBeenCalled();
      });
    });
    ...
    describe("when blurring size", () => {
      ...
      it("should dispatch a size input blur", () => {
        const dispatchEditSizeInputBlurSpy = jest.fn();
        ...
        renderConnectedExchangeInlineEdit({
          ...
          currencySymbol: "€",
          price: 1.23,
          dispatchEditSizeInputBlur: dispatchEditSizeInputBlurSpy,
        });
        const [ExchangeInlineEditPanelInitial] = ExchangeInlineEditPanel.mock.calls[0];
        ...
        expect(dispatchEditSizeInputBlurSpy).toHaveBeenCalledWith("urn", "BET_ID", ExchangeSide.LAY, 1.23, 4);
      });

      it("should not dispatch when there is no size error", () => {
        const dispatchEditSizeInputBlurSpy = jest.fn();
        ...
        expect(dispatchEditSizeInputBlurSpy).not.toHaveBeenCalled();
      });
    ...
    describe("when nudging price", () => {
      describe("when down", () => {
        ...
        it("should not dispatch when there is no runner", () => {
          const dispatchEditPriceNudgeDownSpy = jest.fn();
          ...
          expect(dispatchEditPriceNudgeDownSpy).not.toHaveBeenCalled();
        });
      ...
      describe("when up", () => {
        ...
        it("should not dispatch when there is no runner", () => {
          ...
          expect(dispatchEditPriceNudgeUpSpy).not.toHaveBeenCalled();
        });
    ...
    describe("when nudging size", () => {
      describe("when down", () => {
        ...
        it("should not dispatch when there is no runner", () => {
          ...
          expect(dispatchEditSizeNudgeDownSpy).not.toHaveBeenCalled();
        });
      ...
      describe("when up", () => {
        ...
        it("should not dispatch when there is no runner", () => {
          ...
          expect(dispatchEditSizeNudgeUpSpy).not.toHaveBeenCalled();
        });
    ...
    describe("when focusing price", () => {
      ...
      describe("when called after nudge press", () => {
        beforeEach(() => {
          const { onPriceNudgeUp, onPriceFocus } = ExchangeInlineEditPanel.mock.calls[0][0];
      ...
      describe("when called on input press", () => {
        ...
        it("should clear notifications and clear the input value", () => {
          expect(clearNotifications).toHaveBeenCalledWith(NotificationKeys.Price);
          expect(dispatchEditPriceInputChange).toHaveBeenCalledWith("urn", "BET_ID", "LAY", undefined, 1);
    ...
    describe("when focusing size", () => {
      ...
      describe("when called after nudge press", () => {
        beforeEach(() => {
          const { onSizeNudgeUp, onSizeFocus } = ExchangeInlineEditPanel.mock.calls[0][0];
    ...
    describe("when toggling persistence", () => {
      ...
      it("should not dispatch when there is no betId", () => {
        const dispatchUnmatchedPersistenceListClickSpy = jest.fn();
        ...
        expect(dispatchUnmatchedPersistenceListClickSpy).not.toHaveBeenCalled();
      });
    });
    ...
    describe("when updating", () => {
      it("should dispatch an update action when onUpdate is called", () => {
        ...
        expect(dispatchUnmatchedUpdateSpy).toHaveBeenCalledWith("BET_ID", "market:urn", "urn", null);
      });

      it("should not dispatch an update action when market is missing", () => {
        const dispatchUnmatchedUpdateSpy = jest.fn();
        ...
        expect(dispatchUnmatchedUpdateSpy).not.toHaveBeenCalled();
      });

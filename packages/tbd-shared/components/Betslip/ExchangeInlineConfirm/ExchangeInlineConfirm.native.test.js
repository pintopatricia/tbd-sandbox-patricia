...
import { ExchangeInlineConfirm } from "./ExchangeInlineConfirm.native";
...
jest.mock("./snowflakes/ExchangeInlineConfirmPanel/ExchangeInlineConfirmPanel.native", () => ({
  ExchangeInlineConfirmPanel: jest.fn(() => <exchange-confirm-panel-mock />),
}));
jest.mock("../../../config/endpoints", () => ({
  getEndpoint: jest.fn().mockReturnValue("deposit.endpoint"),
}));
...
describe("ExchangeInlineConfirm", () => {
  ...
  describe("when there is no side", () => {
    ...
    it("should render nothing", () => {
      ...
      expect(betslip.toJSON()).toBeNull();
    });
  });
  ...
  describe("when there is all data", () => {
    it("should render ExchangeInlineConfirmPanel", () => {
      ...
      expect(Styled).toHaveBeenCalledWith(
        { translation: "I18N.BETSLIP.CONFIRM_BET", styles: { depositTo: typography["typography-h220"] } },
        undefined,
      );
      expect(ExchangeInlineConfirmPanel).toHaveBeenCalledTimes(1);
    });
    ...
    describe("and onConfirm is called", () => {
      ...
      describe("when there is a deposit needed", () => {
        ...
        it("should call navigateDeposit", () => {
          ...
          expect(navigateDeposit).toHaveBeenCalledWith("deposit.endpoint");

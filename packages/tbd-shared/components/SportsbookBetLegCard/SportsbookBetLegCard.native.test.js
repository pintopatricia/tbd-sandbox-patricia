...
import { BetSelectionDetails, Divider, TrapWrapper } from "@ppb/the-wall-native";
...
import SportsbookBetLegCard from "./SportsbookBetLegCard.native";
...
jest.mock("../EnhancedTracking/EnhancedTracking.native", () => jest.fn(() => <enhanced-tracking-mock />));

jest.mock("../ObbEnhancedTracking/ObbEnhancedTracking.web", () => jest.fn(() => <obb-enhanced-tracking-mock />));

jest.mock("@ppb/the-wall-common/base-theme", () => ({
  spacings: { "spacing-2": "foo" },
  tokens: {
    BetSelectionDetailsSportsIconWonColour: "BetSelectionDetailsSportsIconWonColour",
    BetSelectionDetailsSportsIconLostColour: "BetSelectionDetailsSportsIconLostColour",
    BetSelectionDetailsSportsIconNeutralColour: "BetSelectionDetailsSportsIconNeutralColour",
  },
}));
...
describe("SportsbookBetLegCard", () => {
  ...
  describe("When the Leg Part has expressionComponents", () => {
    ...
    describe("And the bet is voided", () => {
      it("should render the obb enhanced tracking component", () => {
        ...
        const legPartsWithExpressionComponents = { ...props.legParts[0], expressionComponents, result: Result.VOID };
        const { outcomeDefinitionExp, ...legPartsWithoutOutcomeDefinitionExp } = legPartsWithExpressionComponents;
        renderSportsbookBetLegCard({ legParts: [legPartsWithoutOutcomeDefinitionExp] });
  ...
  describe("when isSuperSub, fixtureURN, fixtureTypename and participantId are defined", () => {
    ...
    it("should call dispatchSubscribeFixtureUpdates on mount", () => {
      renderSportsbookBetLegCard({ legParts: [{ ...props.legParts[0], ...fixtureUpdatesProps }] });
    ...
    it("should call dispatchUnsubscribeFixtureUpdates on unmount", () => {
      const { unmount } = renderSportsbookBetLegCard({ legParts: [{ ...props.legParts[0], ...fixtureUpdatesProps }] });
  ...
  describe("when selectionTypeIcon is provided", () => {
    it("should render the component with selectionTypeIcon", () => {
      renderSportsbookBetLegCard({ legParts: [{ ...props.legParts[0], selectionTypeIcon: "fakeIcon" }] });
  ...
  describe("when onNavigationPress is triggered", () => {
    describe("and there is navigationViewLink and is not BetSharingView", () => {
      it("should dispatch ClickEventAction and navigate properly", () => {
        MOCK_VIEW_URN = ["123"];
        renderSportsbookBetLegCard();
        const { onNavigationPress } = BetSelectionDetails.mock.calls[0][0];
        onNavigationPress();

        expect(navigate).toHaveBeenCalledWith(props.legParts[0].navigationViewLink);
        expect(props.dispatchNavigateToViewLinkAction).toHaveBeenCalledWith("navigationViewLinkUrl", "");
      });
    });

    describe("and there isn't navigationViewLink and is BetSharingView", () => {
      it("should not dispatch any action", () => {
        MOCK_VIEW_URN = ["ppb:tbd:view:generic:betSharing"];
        renderSportsbookBetLegCard({ legParts: [{ ...props.legParts[0], navigationViewLink: null }] });
        const { onNavigationPress } = BetSelectionDetails.mock.calls[0][0];
        onNavigationPress();

        expect(navigate).not.toHaveBeenCalled();
        expect(props.dispatchNavigateToViewLinkAction).not.toHaveBeenCalled();
      });
    });
  });
  ...
  describe("when sport icon component is visible", () => {
    describe.each([Result.WON, Result.WINNING])("and when result is %s", (result) => {
      it("should render the sport icon component with token BetSelectionDetailsSportsIconWonColour", () => {
        renderSportsbookBetLegCard({ legParts: [{ ...props.legParts[0], result }] });

        expect(SportIcon).toHaveBeenCalledWith(
          {
            sportId: "1",
            color: "BetSelectionDetailsSportsIconWonColour",
          },
          undefined,
        );
      });
    ...
    describe.each([Result.LOST, Result.LOSING])("and when result is %s", (result) => {
      it("should render the sport icon component with token BetSelectionDetailsSportsIconLostColour", () => {
        renderSportsbookBetLegCard({ legParts: [{ ...props.legParts[0], result }] });

        expect(SportIcon).toHaveBeenCalledWith(
          {
            sportId: "1",
            color: "BetSelectionDetailsSportsIconLostColour",
          },
          undefined,
        );
      });
    ...
    it("should render the sport icon component with token BetSelectionDetailsSportsIconNeutralColour when result is none of the above", () => {
      renderSportsbookBetLegCard({ legParts: [{ ...props.legParts[0], result: undefined }] });

      expect(SportIcon).toHaveBeenCalledWith(
        {
          sportId: "1",
          color: "BetSelectionDetailsSportsIconNeutralColour",
        },
        undefined,
      );
    });

...
import FreezeSelection from "./FreezeSelection.web";
import FreezeSelectionBottomSheet from "./FreezeSelectionBottomSheet";

jest.mock("../../helpers/i18n", () => ({ i18n: jest.fn(({ key }) => key) }));

jest.mock("@ppb/the-wall-web", () => ({
  SecondaryButton: jest.fn(({ onTap }) => (
    <secondary-button-mock onClick={onTap} data-testid="freeze-selection-button" />
  )),
  Alert: jest.fn(() => <alert-component></alert-component>),
}));

jest.mock("./FreezeSelectionBottomSheet", () =>
  jest.fn((props) => <freeze-selection-bottom-sheet-mock data-testid="freeze-selection-bottom-sheet" {...props} />),
);
...
describe("FreezeSelection", () => {
  ...
  it("should open the bottom sheet when the button is clicked", async () => {
    ...
    fireEvent.click(getByTestId("freeze-selection-button"));
    await waitFor(() => {

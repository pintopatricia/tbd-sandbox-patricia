...
import { createAustralianRulesFixtureViewModel } from "../../view-model-factories/australian-rules-fixture";
...
export const makeMapStateToProps: MapStateToPropsFactory<StateProps, ContainerProps, ApplicationState> = () => {
  ...
  return function mapStateToProps(
    ...
    let dateTime;
    let time;
    let matchStatus: MatchStatus = MatchStatus.PRE_MATCH;
    ...
    const australianRulesFixture = getAustralianRulesFixtureByURN(state.entities.australianrulesfixtures, urn);

    if (!australianRulesFixture) return {};

    const { score, periodScores, opponentsNames, isAmericanFormat } = australianRulesFixture;
    ...
    const australianRulesScoreboard = getAustralianRulesFixtureViewModel({
      userDetails,
      sportEvent,
      score,
      periodScores,
    ...
    if (australianRulesScoreboard) {
      ({ date, dateTime, time, scoreData, matchStatus } = australianRulesScoreboard);
    }

...
import { BaseballClock } from "@ppb/tbd-store/state/entities/baseball-fixture/BaseballFixture";
...
export const makeMapStateToProps: MapStateToPropsFactory<StateProps, ContainerProps, ApplicationState> = () => {
  ...
  return function mapStateToProps(
    ...
    const baseballFixture = getBaseballFixtureByURN(state.entities.baseballfixtures, urn);

    if (!baseballFixture) return {};

    const { clock, score, scorePerInning, opponentsNames, isAmericanFormat } = baseballFixture;
    ...
    const baseballScoreboard = getBaseballFixtureViewModel({
      userDetails,
      sportEvent,
      scorePerInning,
      score,
    ...
    if (baseballScoreboard) {
      ({ date, dateTime, time, scoreData, matchStatus } = baseballScoreboard);
    }

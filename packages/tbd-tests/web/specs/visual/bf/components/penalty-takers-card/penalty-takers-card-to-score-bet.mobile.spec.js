...
const { PenaltyTakersCardPO, SportsbookPlacePanelPO, SportsbookBetButtonPO } = require("../../../../../page-objects");
...
const MODULE_NAME = "penalty_takers_card_to_score_bet";
...
describe("Penalty Takers Card — To Score bet", () => {
  describe("Given I'm on a football event page", () => {
    ...
    describe("When I click on a home player from the carousel", () => {
      beforeAll(async () => {
        await penaltyTakersCardPO.swipeLeft();

        const toScoreTopLeftSportsbookBetButtonPO = new SportsbookBetButtonPO(penaltyTakersCardPO.toScoreMarketTopLeft);

        await browser.waitUntilEquals(toScoreTopLeftSportsbookBetButtonPO.odd, "7.5");

        await browser.waitUntilImageEquals(
          `${MODULE_NAME}_[PRPI-11910]_then_a_visual_goal_representation_is_displayed`,
        );
      });

      it("[PRPI-11910]_then_a_visual_goal_representation_is_displayed", async () => {
        expect(
          await browser.checkScreen(`${MODULE_NAME}_[PRPI-11910]_then_a_visual_goal_representation_is_displayed`),
        ).toBe(0);
      });
    });
    ...
    describe("When I click a zone/selection in the goal representation", () => {
      beforeAll(async () => {
        await penaltyTakersCardPO.toScoreMarketTopLeft.click();
        await browser.waitUntilDisplayed(placePanelPO.element);
        await browser.waitUntilImageEquals(
          `${MODULE_NAME}_[PRPI-11911]_then_the_selected_To_Score_selection_is_automatically_added_to_the_betslip`,
        );
      });

      it("[PRPI-11911]_then_the_selected_To_Score_selection_is_automatically_added_to_the_betslip", async () => {
        expect(

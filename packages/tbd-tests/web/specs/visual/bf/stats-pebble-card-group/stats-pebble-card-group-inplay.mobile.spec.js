const { SupportingContentButtonPO } = require("../../../../page-objects");
...
const MODULE_NAME = "stats_pebble_card_group_in_play";
...
describe("Football stats in Coupon Card Group", () => {
  ...
  describe("when BFF returns a stats card group to the coupon", () => {
    beforeAll(async () => {
      await browser.waitUntilImageEquals(`${MODULE_NAME}_[PRPI-1564]_should_correctly_display_the_coupon`);
    });

    it("[PRPI-1564]_should_correctly_display_the_coupon", async () => {
      expect(await browser.checkScreen(`${MODULE_NAME}_[PRPI-1564]_should_correctly_display_the_coupon`)).toBe(0);
    });
      ...
      describe("and there is SCA information", () => {
        ...
        beforeAll(async () => {
          ...
          await browser.waitUntilImageEquals(
            `${MODULE_NAME}_[PRPI-1565]_should_correctly_display_the_stats_opened_in_coupon`,
          );
        });

        it("[PRPI-1565]_should_correctly_display_the_stats_opened_in_coupon", async () => {
          expect(
            await browser.checkScreen(`${MODULE_NAME}_[PRPI-1565]_should_correctly_display_the_stats_opened_in_coupon`),
          ).toBe(0);
        });
      });
    });

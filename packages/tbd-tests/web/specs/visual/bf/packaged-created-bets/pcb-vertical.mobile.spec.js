...
const PackagedCreatedBetsCardPO = require("@ppb/tbd-shared/components/PackagedCreatedBetsCard/PackagedCreatedBetsCard.web.po");
...
const pcbPO = new PackagedCreatedBetsCardPO();
...
const SIB_SHOW_MORE_MOCK = {
  betCombinations: [OP_FIFTH_COMBINATION],
  runnerOdds: [NINTH_RUNNER_SIB, TENTH_RUNNER_SIB],
};
...
describe("PackagedCreatedBetsCard", () => {
  describe("Vertical List Card", () => {
    beforeAll(async () => {
      await mockService.mockHttpRequest(await getIndexHTML(BFF_VIEW_MOCK.urn, { disableFakeTimers: true }));
      ...
      await browser.waitUntilImageEquals(`${MODULE_NAME}_[PRPI-1495]_should_render_vertical_card`);
    });

    it("[PRPI-1495]_should_render_vertical_card", async () => {
      expect(await browser.checkScreen(`${MODULE_NAME}_[PRPI-1495]_should_render_vertical_card`)).toBe(0);
    });
    ...
    describe("Show More", () => {
      beforeAll(async () => {
        ...
        await browser.waitUntilImageEquals(`${MODULE_NAME}_[PRPI-1496]_should_render_vertical_card_expanded`);
      });

      it("[PRPI-1496]_should_render_vertical_card_expanded", async () => {
        expect(await browser.checkScreen(`${MODULE_NAME}_[PRPI-1496]_should_render_vertical_card_expanded`)).toBe(0);
      });
    });
  });
});

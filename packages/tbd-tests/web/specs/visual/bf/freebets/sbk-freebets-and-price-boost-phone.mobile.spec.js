...
describe("Freebets and price boost warning message", () => {
  beforeAll(async () => {
  ...
  describe("when user adds three selections", () => {
    ...
    describe("and the user clicks on the free bets checkbox", () => {
      beforeAll(async () => {
        await freeBetsPO.activateBonus("Use Free Bet Balance");
        await stakeInputFieldPO.element.scrollIntoView({ block: "start" });
        await addStake(stakeInputFieldPO, "2");
      });

      describe("and the user clicks on Generosity Wallet Button", () => {
        beforeAll(async () => {
          ...
          await browser.waitUntil(
            async () => browser.containsClass(multipleGenerosityWalletButtonPO.element, PromoButtonPO.states.selected),
            {
              timeoutMsg: "Price Boost was not selected",
            },
          );

          await browser.waitUntilNotDisplayed(keyboardPO.element, "keyboard should not be displayed");
          await multipleGenerosityWalletButtonPO.element.scrollIntoView({ block: "start" });
          await browser.waitUntilImageEquals(
            `${MODULE_NAME}_[PRPI-4694]_should_see_the_free_bet_ineligible_message_when_price_boost_is_selected`,
          );
        });
        it("[PRPI-4694]_should_see_the_free_bet_ineligible_message_when_price_boost_is_selected", async () => {
          expect(

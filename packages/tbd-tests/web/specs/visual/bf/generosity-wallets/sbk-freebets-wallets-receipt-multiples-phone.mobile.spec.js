...
const doubleGenerosityWalletButtonPO = new PromoButtonPO(doubleBetControlsPO.generosityWalletButton);
...
const multipleGenerosityWalletsAlertPO = new AlertPO(sportsbookReceiptPanelPO.multiples[0]);
const firstSingleGenerosityWalletsAlertPO = new AlertPO(sportsbookReceiptPanelPO.singles[0]);
const secondSingleGenerosityWalletsAlertPO = new AlertPO(sportsbookReceiptPanelPO.singles[1]);
...
describe("SBK Free Bets Wallets - Multiples Receipt", () => {
  beforeAll(async () => {
    ...
    await doubleGenerosityWalletButtonPO.element.waitForClickable();
    await doubleGenerosityWalletButtonPO.element.click();
    ...
    await browser.waitUntilDisplayed(multipleGenerosityWalletsAlertPO.message);
    await browser.waitUntilImageEquals(
      `${MODULE_NAME}_[PRPI-1358]_should_display_the_multiples_receipt_free_bets_wallets_generosity_alert`,
    );
  });

  it("[PRPI-1358]_should_display_the_multiples_receipt_free_bets_wallets_generosity_alert", async () => {
    expect(
  ...
  describe("then the user scrolls down", () => {
    beforeAll(async () => {
      await secondSingleGenerosityWalletsAlertPO.message.scrollIntoView({
        block: "center",
      });
      await browser.waitUntilDisplayed(firstSingleGenerosityWalletsAlertPO.message);
      await browser.waitUntilDisplayed(secondSingleGenerosityWalletsAlertPO.message);
      await browser.waitUntilImageEquals(
        `${MODULE_NAME}_[PRPI-1359]_should_display_the_singles_free_bets_wallets_generosity_alert_on_the_receipt`,
      );
    });

    it("[PRPI-1359]_should_display_the_singles_free_bets_wallets_generosity_alert_on_the_receipt", async () => {
      expect(
        await browser.checkScreen(
          `${MODULE_NAME}_[PRPI-1359]_should_display_the_singles_free_bets_wallets_generosity_alert_on_the_receipt`,
        ),

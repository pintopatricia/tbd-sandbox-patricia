...
describe("SBK Free Bets Wallets - Multiples Confirm", () => {
  beforeAll(async () => {
    ...
    await browser.waitUntilDisplayed(
      firstSingleFreeBetsWalletsAlertPO.message,
      "First Single Free Bets Wallets Alert is not displayed",
    );
    await browser.waitUntilDisplayed(
      secondSingleFreeBetsWalletsAlertPO.message,
  ...
  describe("on multiples and place footer", () => {
    beforeAll(async () => {
      await browser.waitUntilDisplayed(doubleFreeBetsWalletsAlertPO.element, "DOUBLE Free Bets Alert is not displayed");
    });

    afterAll(async () => {
      await multiplesCardPO.header.waitForClickable();
      await multiplesCardPO.header.click();
      await browser.waitUntilNotDisplayed(multiplesCardPO.content, "Multiples section is not collapsed");
    });

    it("[PRPI-1352]_should_display_the_confirm_step_multiples_combinations_with_the_selected_wallets_amount_and_the_place_footer_total_selected_wallets_amount", async () => {
      expect(
        await browser.checkScreen(
          `${MODULE_NAME}_[PRPI-1352]_should_display_the_confirm_step_multiples_combinations_with_the_selected_wallets_amount_and_the_place_footer_total_selected_wallets_amount`,
        ),
      ).toBe(0);
    });
  });
  ...
  describe("on singles", () => {
    it("[PRPI-1353]_should_display_the_confirm_step_multiples_single_combinations_with_the_selected_wallets_amount", async () => {
      expect(
        await browser.checkScreen(
          `${MODULE_NAME}_[PRPI-1353]_should_display_the_confirm_step_multiples_single_combinations_with_the_selected_wallets_amount`,
        ),
      ).toBe(0);
    });
  });

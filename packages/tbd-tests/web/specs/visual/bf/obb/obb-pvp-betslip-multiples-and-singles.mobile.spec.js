...
const betControls = new BetControlsPO(firstSingle.element);
const singleStakeInput = new CurrencyNumberInputFieldPO(betControls.currencyInput);
...
const MODULE_NAME = "obb_pvp_betslip_multiples_and_singles";

describe("OBB - Mutiples and Singles - Place and receipt", () => {
  describe("when placing a multiple and a single bet with success", () => {
    beforeAll(async () => {
    ...
    describe("and when I enter stake for the multiple and for the singles", () => {
      beforeAll(async () => {
        await stakeInputFieldPO.element.waitForClickable();
        await stakeInputFieldPO.element.click();
        await stakeInputFieldPO.setValue("22");

        await multiplesCardPO.header.click(); // close multiples
        await singlesCardPO.header.click(); // open singles
        await singleStakeInput.setValue("1"); // fill stake for the first single
      });

      it("[PRPI-1640] place button should combine both stakes", async () => {
        expect(await placeButtonPO.label.getText()).toBe("Place $23.00 Bet");
      });

      describe("and when I place the multiple bet", () => {
        beforeAll(async () => {
          await placeButtonPO.element.click();
          await browser.waitUntilImageEquals(`${MODULE_NAME}_[PRPI-1477]_should_see_multiples_and_singles_receipt`);
        });

        it("[PRPI-1477]_should_see_multiples_and_singles_receipt", async () => {
          expect(await browser.checkScreen(`${MODULE_NAME}_[PRPI-1477]_should_see_multiples_and_singles_receipt`)).toBe(
            0,
          );
        });
      });

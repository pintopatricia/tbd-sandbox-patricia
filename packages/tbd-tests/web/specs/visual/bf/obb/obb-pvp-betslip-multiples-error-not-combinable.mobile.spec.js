...
const shotsPvpCardSecondBetButton = new SportsbookBetButtonPO(shotsPvpCardPo.secondBetButton);
  ...
  combinedBetDefinitions: [
    {
      result: {
        ...
        errorDetails: "Error quoting expression",
      ...
      legs: [
        {
          ...
          betDefinitions: ["529aedc4c4fbc0b0", "57068fc434f127dc"],
          result: {
            errorDetails: "Error quoting expression",
            resultCode: "IMPOSSIBLE_OBB_CHOICE",
            __typename: "ObbResult",
...
const MODULE_NAME = "obb_pvp_betslip_multiples_error_not_combinable";

describe("OBB - Mutiples - Bets can't be combined", () => {
  describe("when placing a multiple bet but legs are not combinable", () => {
    beforeAll(async () => {
      ...
      // closing betslip and adding another selection in the first pvp card (counter bet)
      await betslipDrawerPO.header.click();
      await shotsPvpCardSecondBetButton.element.scrollIntoView({ block: "center" });
      await shotsPvpCardSecondBetButton.element.click();
    });

    it("[PRPI-1641] the minimized betslip should state that some selections cannot be combined", async () => {
      expect(await minimizedPO.title.getText()).toBe("Some selections cannot be combined");
    });

    it("[PRPI-1642] the minimized betslip counter should be 2", async () => {
      expect(await minimizedPO.counter.getText()).toBe("2");
    });

    describe("when I open the betslip", () => {
      beforeAll(async () => {
        // expanding betslip
        await minimizedPO.title.click();
        await browser.waitUntilDisplayed(sportsbookPlacePanelPO.element);
        await browser.waitUntilImageEquals(`${MODULE_NAME}_[PRPI-1478]_the_multiples_section_should_not_be_editable`);
      });

      it("[PRPI-1478]_the_multiples_section_should_not_be_editable", async () => {
        expect(
          await browser.checkScreen(`${MODULE_NAME}_[PRPI-1478]_the_multiples_section_should_not_be_editable`),
        ).toBe(0);
      });
    });
  });

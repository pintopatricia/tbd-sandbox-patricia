...
const EVENT_TYPE_ID_BASKETBALL = 7522;
const EVENT_ID_BASKETBALL = 123456;
...
const BFF_BASKETBALL_VIEW_MOCK = {
  urn: `ppb:tbd:view:sport:${EVENT_TYPE_ID_BASKETBALL}`,
  edges: [
    {
      node: {
          ...
          edges: [
            {
              node: {
                ...
                fixture: {
                  ...
                  urn: `ppb:fixture:${EVENT_ID_BASKETBALL}`,
                  periodClock: "UNKNOWN_PERIOD",
                },
                viewLink: {
                  viewUrn: `ppb:tbd:view:event:${EVENT_ID_BASKETBALL}`,
                  viewUrl: `basketball/nba/los-angeles-lakers-v-chicago-bulls/e-${EVENT_ID_BASKETBALL}`,
                },
                  ...
                  selections: [
                    {
                      market: {
                        ...
                        hierarchy: {
                          ...
                          sportevent: {
                            urn: `ppb:event:${EVENT_ID_BASKETBALL}`,
                          },
                        },
                        runners: [
                          {
...
describe("PopularBetBuilder", () => {
  describe("Basketball Bet Builder", () => {
    beforeAll(async () => {
      await mockService.mockHttpRequest(await getIndexHTML(BFF_BASKETBALL_VIEW_MOCK.urn));
      await mockService.mockHttpRequest(getImplyBetsResponse(SIB_MOCK, { ignoreLegsOrder: true }));
      await mockService.mockHttpRequest(getMarketPrices(SMP_MOCK));
      await mockService.mockHttpRequest(getSportsLayout(BFF_BASKETBALL_VIEW_MOCK));
      await mockService.mockHttpRequest(getMarkets(GET_MARKETS_MOCK));
      await mockService.mockFonts(getMockFonts());
      await browser.url(routes.getSportViewUrl(EVENT_TYPE_ID_BASKETBALL));
      await browser.waitUntilEquals(popularBetBuilderPO.popularBetBuilderBetButton, "Add to Betslip at 81.81");
      await browser.waitUntilImageEquals(
        `${MODULE_NAME}_[PRPI-1503]_should_render_basketball_popular_bet_builder_card`,
      );
    });

    it("[PRPI-1503]_should_render_basketball_popular_bet_builder_card", async () => {
      expect(
        await browser.checkScreen(`${MODULE_NAME}_[PRPI-1503]_should_render_basketball_popular_bet_builder_card`),
      ).toBe(0);
    });
  });
});

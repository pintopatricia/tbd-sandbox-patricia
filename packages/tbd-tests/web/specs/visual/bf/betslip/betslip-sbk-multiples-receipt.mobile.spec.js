...
const receiptCompositionPO = new BetSelectionsPO(sportsbookReceiptPanelPO.element);
...
const multiLinesMultiplesPO = new MultiLinesMultiplesPO(sportsbookPlacePanelPO.element);
const firstAdditionalMultipleControlsPO = new BetControlsPO(multiLinesMultiplesPO.multiples[0]);
const firstAdditionalMultipleStakeFieldPO = new CurrencyNumberInputFieldPO(
  ...
  result: [
    {
      ...
      legs: [
        ...
        {
          leg: {
            betRunners: [{ runner: { marketId: "924.3", selectionId: 1 } }],
          },
          winOdds: {
            decimalDisplayOdds: { decimalOdds: 2 },
            fractionalDisplayOdds: { numerator: 1, denominator: 1 },
...
describe("Multiples - Receipt Panel", () => {
  describe("when having multiple selections in the betslip", () => {
    beforeAll(async () => {
    ...
    describe("when first selection has been added", () => {
      beforeAll(async () => {
      ...
      describe("when second selection has been added", () => {
        beforeAll(async () => {
        ...
        describe("when third selection has been added", () => {
          beforeAll(async () => {
          ...
          describe("When staking and placing the multiples", () => {
            beforeAll(async () => {
              await sportsbookMinimizedBetslipPO.element.waitForClickable();
              await sportsbookMinimizedBetslipPO.element.click();
              await browser.waitUntilInViewport(sportsbookPlacePanelPO.element);

              await browser.waitUntilDisplayed(
                sportsbookPlacePanelPO.element,
                "Multiples place panel couldn't be opened",
              );

              await oneLineMultipleStakeFieldPO.numberField.waitForClickable();
              await oneLineMultipleStakeFieldPO.numberField.click();
              await browser.waitUntil(() => oneLineMultipleStakeFieldPO.numberField.isFocused());

              await addStake(oneLineMultipleStakeFieldPO, "0.12");

              await firstAdditionalMultipleStakeFieldPO.element.scrollIntoView({ block: "center" });
              await firstAdditionalMultipleStakeFieldPO.numberField.waitForClickable();
              await firstAdditionalMultipleStakeFieldPO.numberField.click();
              await browser.waitUntil(() => firstAdditionalMultipleStakeFieldPO.numberField.isFocused());

              await addStake(firstAdditionalMultipleStakeFieldPO, "0.12");
              ...
              await browser.waitUntilImageEquals(
                `${MODULE_NAME}_[PRPI-1257]_should_have_expanded_accordion_with_selections_with_placed_multiples`,
              );
            });

            it("[PRPI-1257]_should_have_expanded_accordion_with_selections_with_placed_multiples", async () => {
              expect(
    ...
    describe("when the collapse is expanded", () => {
      beforeAll(async () => {
        await browser.waitUntilDisplayed(
          sportsbookReceiptPanelPO.element,
          "Multiples receipt panel couldn't be closed",
        );
        await receiptCompositionPO.header.waitForClickable();
        await receiptCompositionPO.header.click();
        await browser.waitUntilImageEquals(
          `${MODULE_NAME}_[PRPI-1258]_should_have_collapsed_accordion_with_placed_multiples`,
        );
      });

      it("[PRPI-1258]_should_have_collapsed_accordion_with_placed_multiples", async () => {
        expect(
          await browser.checkScreen(`${MODULE_NAME}_[PRPI-1258]_should_have_collapsed_accordion_with_placed_multiples`),
        ).toBe(0);
      });
    });
  });

...
describe("Inline Betslip - Unmatched Receipt", () => {
  ...
  describe("when the user places a back bet and it is unmatched", () => {
    beforeAll(async () => {
      ...
      await mockService.mockHttpRequest(getPlaceBetResponse(ETX_BACK_UNMATCHED_MOCK));
      ...
      await browser.waitUntilEquals(exchangeUnmatchedCardPO.header, "Bet Unmatched");
      await browser.waitUntilImageEquals(`${MODULE_NAME}_[PRPI-10440]_should_display_the_unmatched_receipt`);
    });

    it("[PRPI-10440]_should_display_the_unmatched_receipt", async () => {
      expect(await browser.checkScreen(`${MODULE_NAME}_[PRPI-10440]_should_display_the_unmatched_receipt`)).toBe(0);
    });
  });
});

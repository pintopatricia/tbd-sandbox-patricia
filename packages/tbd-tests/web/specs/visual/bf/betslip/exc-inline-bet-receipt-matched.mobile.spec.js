...
describe("Inline Betslip - Matched Receipt", () => {
  beforeAll(async () => {
  ...
  describe("when the user places a back bet and it gets matched", () => {
    beforeAll(async () => {
      ...
      await browser.waitUntilImageEquals(`${MODULE_NAME}_[PRPI-10438]_should_display_the_matched_receipt`);
    });

    it("[PRPI-10438]_should_display_the_matched_receipt", async () => {
      expect(await browser.checkScreen(`${MODULE_NAME}_[PRPI-10438]_should_display_the_matched_receipt`)).toBe(0);
    });
  });
});

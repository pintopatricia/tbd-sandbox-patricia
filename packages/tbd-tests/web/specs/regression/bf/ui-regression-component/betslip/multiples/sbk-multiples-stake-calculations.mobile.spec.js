...
const betsSummaryPO = new BetsSummaryPO(sportsbookPlacePanelPO.totalSummary);
...
describe("Sportsbook stake calculations", () => {
  describe("when adding two selections to betslip", () => {
    ...
    it("[PRPI-8198] should open betslip multiples panel", async () => {
      expect(await sportsbookPlacePanelPO.element.isDisplayedInViewport()).toBe(true);
    });

    it("[PRPI-8199] should have a 'Double' multiple", async () => {
    ...
    describe("when I input 2 on the size input field", () => {
      beforeAll(async () => {
        ...
        await stakeFieldPO.element.click();
        await stakeFieldPO.setValue("2");
      });

      it("[PRPI-8200] should show Returns label", async () => {
        expect(await multipleControlsPO.returnsLabel.getText()).toBe("Returns");
        expect(await multipleControlsPO.returnsValueContainer.getText()).toBe("$4.80");
      });

      it("[PRPI-8201] should enable total stake with value", async () => {
        expect(await betsSummaryPO.element.getAttribute("aria-disabled")).toBe("false");

        expect(await placeButtonPO.label.getText()).toContain("$2.00");
      });

      it("[PRPI-8202] should enable potential returns with value", async () => {
        expect(await betsSummaryPO.element.getAttribute("aria-disabled")).toBe("false");

        expect(await betsSummaryPO.totalReturnsLabel.getText()).toBe("Total Returns");
        expect(await betsSummaryPO.totalReturnsValue.getText()).toBe("$4.80");
      });
      ...
      describe("when I add another selection", () => {
        beforeAll(async () => {
          ...
          await browser.waitUntilNotDisplayed(
            ...
            "Multiples place panel hasn't been minimized",
          );
          ...
          await browser.waitUntilDisplayed(
            ...
            "Fourth runner bet button not visible",
          );
        ...
        it("[PRPI-8203] should clear the stake value", async () => {
          expect(await stakeFieldPO.numberField.getValue()).toBe("");
        });

        it("[PRPI-8204] should disable Total stake without value", async () => {
        ...
        it("[PRPI-8205] should disable Potential returns without value", async () => {
          expect(await betsSummaryPO.element.getAttribute("aria-disabled")).toBe("true");

...
const oddsInputField = new FixedNumberInputFieldPO(sportsbookMultiplesControlsPO.element);
const betslipDrawerPO = new BetslipDrawerPO();
const selectionsBoardPO = new SelectionsBoardPO(sportsbookPlacePanelPO.element);
...
describe("Sportsbook multiples", () => {
  describe("when placing a multiple bet", () => {
    beforeAll(async () => {
      ...
      await browser.waitUntilInViewport(selectionsBoardPO.element);
    });

    it("[PRPI-8192] should display 2 selections expanded", async () => {
      expect(await selectionsBoardPO.title.getText()).toBe("2 Selections");
    });

    it("[PRPI-8193] should display a 'Double' multiple", async () => {
      expect(await oneLineMultiplePO.text.getText()).toBe("Double");
    });

    it("[PRPI-8194] should display the combination odd", async () => {
      expect(await oddsInputField.numberField.getValue()).toBe("2.4");
    });

    it("[PRPI-8195] should display the total stake on the bet button", async () => {
      expect(await placeButtonPO.label.getText()).toBe("Please Enter Stake");
    });

    it("[PRPI-8196] should display the balance after bet", async () => {
    ...
    it("[PRPI-8197] should display the potential returns of the combination", async () => {

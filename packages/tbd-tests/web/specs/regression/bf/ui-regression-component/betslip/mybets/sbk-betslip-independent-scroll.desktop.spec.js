...
const desktopBetsliPO = new DesktopBetslipPO();
...
describe("Sportsbook multiples", () => {
  describe("When the user has a multiple bet on the bet slip and scrolls the mouse over the bet slip.", () => {
    beforeAll(async () => {
    ...
    it("[PRPI-5261]Left side bar should be sticky", async () => {
      const inicialLeftSideBar = await leftSideBarPO.element.getLocation();
      await singlesCardPO.singles[1].scrollIntoView();
      expect(await leftSideBarPO.element.getLocation()).toEqual(inicialLeftSideBar);
    });

    it("[PRPI-5262]Betslip should be sticky", async () => {
      const inicialDesktopHeader = await desktopBetsliPO.title.getLocation();
      await singlesCardPO.singles[1].scrollIntoView();
      expect(await desktopBetsliPO.title.getLocation()).toEqual(inicialDesktopHeader);
    });
  });
});

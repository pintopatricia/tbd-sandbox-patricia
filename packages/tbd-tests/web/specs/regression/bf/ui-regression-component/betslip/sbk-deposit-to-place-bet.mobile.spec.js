...
describe("Betslip - Sportsbook Deposit to Place Bet", () => {
  ...
  describe("When I try to place a single bet with an amount higher than the available wallet funds", () => {
    beforeAll(async () => {
      ...
      await stakeFieldPO.setValue("9");
      await browser.waitUntilDisplayed(alertPO.icon);
    });

    it("[PRPI-5610] The betslip should display CTA button with the text `Deposit to Place $9.00 Bet`", async () => {
      expect(await placeButtonPO.element.getText()).toBe("Deposit to Place $9.00 Bet");
    });

    it("[PRPI-5611] The betslip should display the Balance After Bet with the text 'N/A'", async () => {
    ...
    it("[PRPI-5612] The betslip should display an insufficient funds notification", async () => {
  ...
  describe("When I try to place a multiple bet with an amount higher than the available wallet funds", () => {
    ...
    it("[PRPI-5613] The betslip should display CTA button with the text `Deposit to Place $9.00 Bet`", async () => {
      expect(await placeButtonPO.element.getText()).toBe("Deposit to Place $9.00 Bet");
    });

    it("[PRPI-5614] The betslip should display the Balance After Bet with the text 'N/A'", async () => {
      expect(await betsSummaryPO.leftSegmentValue.getText()).toBe("N/A");
    });

    it("[PRPI-5615] The betslip should display an insufficient funds notification", async () => {
      expect(await alertPO.icon.isDisplayed()).toBe(true);
      expect(await alertPO.message.getText()).toBe("Not enough money in your main wallet.");
    });
  });

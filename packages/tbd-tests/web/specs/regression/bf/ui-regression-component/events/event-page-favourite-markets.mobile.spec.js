const { CardPO, IconButtonPO, SingleTabPO, TooltipPO } = require("../../../../../page-objects");
const FavouriteMarketsEmptyStatePO = require("@ppb/tbd-shared/components/FavouriteMarketsEmptyState/FavouriteMarketsEmptyState.po");
...
const { getEventLayout, getCardResults, setFavouriteMarket } = require("@ppb/tbd-shared/mocks/bff/bff.controller");
...
const secondSingleTabPO = new SingleTabPO(secondTabPO);

const tooltipPO = new TooltipPO();
const firstPebbleCardGroup = new PebbleCardGroupPO(firstTabPO.element);
const firstIconButtonElement = new CardPO(firstPebbleCardGroup.element);
const firstIconButton = new IconButtonPO(firstIconButtonElement.endElement);

const favouriteMarketsEmptyStatePO = new FavouriteMarketsEmptyStatePO();
...
describe("Event Page - Favourite Markets", () => {
  describe("When the user opens the event page with the Favourites tab available", () => {
    beforeAll(async () => {
      await mockService.mockHttpRequest(await getIndexHTML(BFF_MOCK.urn, APP_CONTEXT_MOCK));
    ...
    it("[PRPI-3734] Should not display the tooltip", async () => {
      expect(await tooltipPO.element.isDisplayed()).toBe(false);
    });

    it("[PRPI-3735] Should display an IconButton on the PebbleCardGroup", async () => {
      expect(await firstIconButton.element.isDisplayed()).toBe(true);
    });

    it("[PRPI-3736] Should display the favourite markets tab correctly", async () => {
      expect(await firstSingleTabPO.icon.isDisplayed()).toBe(true);
      expect(await firstSingleTabPO.title.isDisplayed()).toBe(false);
    });

    // cleanup in #INCGNT-633
    /* describe("And the user clicks on the close button on the Tooltip", () => {
      beforeAll(async () => {
        await tooltipPO.closeButton.waitForClickable();
        await tooltipPO.closeButton.click();

        await browser.waitUntilNotDisplayed(tooltipPO.element, "Tooltip still displayed");
      });
    ...
    describe("When the user navigates to the Favourite tab", () => {
      beforeAll(async () => {
        ...
        await firstSingleTabPO.element.click();

        await browser.containsClass(firstTabPO, NavigationTabsListPO.states.selected);
        await browser.waitUntilDisplayed(favouriteMarketsEmptyStatePO.title);
      });

      it("[PRPI-3738] Should display the Empty State with a title", async () => {
        expect(await favouriteMarketsEmptyStatePO.title.getText()).toBe("Looks like there are no favourite markets...");
      });

      it("[PRPI-3739] Should display the Empty State with a message", async () => {
        expect(await favouriteMarketsEmptyStatePO.message.getText()).toBe(
    ...
    xdescribe("And the user clicks on the Popular's tab and presses on the favourite icon on a PebbleCardGroup and navigates back to the Favourite Tab", () => {
      beforeAll(async () => {
        await secondSingleTabPO.element.click();
        ...
        await firstIconButton.element.click();
        await firstSingleTabPO.element.click();
        await browser.containsClass(firstTabPO, NavigationTabsListPO.states.selected);
      });

      it("[PRPI-5727] Should display the favourited content section", async () => {
        expect(await firstPebbleCardGroup.collapseTitle.getText()).toBe("Popular Pebble Card Group");
      });
    });
  });
});

...
const { SwitchOptionPO, SwitchPO } = require("../../../../../page-objects");
...
const firstPreferenceSingleChoiceCardPO = new PreferenceSingleChoiceCardPO(navigationTabsListPO.tabItems[0]);
const switchOptionPO = new SwitchOptionPO(firstPreferenceSingleChoiceCardPO.element);
...
const setup = async ({ jurisdiction = "INTERNATIONAL", isThrottleActive = true } = {}) => {
  await mockService.mockHttpRequest(
    await getIndexHTML(USER_PREFERENCES_MOCK.urn, {
      loggedIn: "true",
      jurisdiction,
  ...
  describe("when a logged-in user opens the 'Settings' page in 'Betting' section", () => {
    describe("when the 'CONFIRM_CASHOUT_PREFERENCE' throttle is ON", () => {
      describe("when the user is in 'Brazil' jurisdiction", () => {
        beforeAll(async () => {
          await setup({ jurisdiction: "BRAZIL" });
        });

        it("[PRPI-11600] should not display the 'Show Cash Out confirmation' preference", async () => {
          expect(await firstPreferenceSingleChoiceCardPO.headerTitle).not.toBe("Show Cash Out confirmation");
        });
      });
      ...
      describe("when the user is in any other jurisdiction", () => {
        ...
        it("[PRPI-11601] the 'Show Cash Out confirmation' preference should be displayed", async () => {
          expect(await firstPreferenceSingleChoiceCardPO.headerTitle.getText()).toBe("Show Cash Out confirmation");
        });

        it("[PRPI-11602] the preference should be displayed as a switch layout", async () => {
          expect(await switchOptionPO.label.getText()).toBe("Show confirmation during Cash Out process.");
        });
        ...
        describe("when the user toggles on the preference", () => {
          ...
          it("[PRPI-11604] the preference should be toggled on", async () => {
            expect(await switchPO.checkbox.isSelected()).toBe(true);
          });
        });
      });
    });

...
const firstPvpCardPO = new ObbPvpCardPO(obbSectionPO.pvpCards[0]);
const secondPvpCardPO = new ObbPvpCardPO(obbSectionPO.pvpCards[1]);
const thirdPvpCardPO = new ObbPvpCardPO(obbSectionPO.pvpCards[2]);
...
const sliderMinusButtonPO = sliderPO.buttons[0];
const settlementConditionCardPO = new SettlementConditionCardPO();
...
const oddsInputFieldPO = new FixedNumberInputFieldPO(multipleControlsPO.fixedInput);
const multipleReturnValuesPO = new PNLAndWhatIfPO(multipleControlsPO.returnsValueContainer);

// OBB PVP BET BUTTONS
const firstPvpCardFirstBetButton = new SportsbookBetButtonPO(firstPvpCardPO.firstBetButton);
const secondPvpCardSecondBetButton = new SportsbookBetButtonPO(secondPvpCardPO.secondBetButton);
const thirdPvpCardSecondBetButton = new SportsbookBetButtonPO(thirdPvpCardPO.secondBetButton);
                              ...
                              cards: {
                                edges: [
                                  ...
                                  },
                                  {
                                    node: {
                                      __typename: "ObbPvpCard",
                                      urn: "ppb:obb:card:pvp:ZypHBBIAACAAKdaU/e/33755137",
                                      title: { name: "Title" },
                                      event: { urn: "ppb:event:33755137", __typename: "SportsEvent" },
                                      teams: {
                                        home: {
      ...
      legs: [
        {
          ...
          betDefinitions: ["529aedc4c4fbc0b0", "43776dd42d290040", "fd2fc080180da89b"],
          result: {
            errorDetails: null,
            resultCode: "SUCCESS",
            __typename: "ObbResult",
...
const thirdImplyBetsResponseNoPrice = {
  betDefinitions: [
    {
      id: "529aedc4c4fbc0b0",
      details: {
...
describe("OBB - x of n - Place and receipt", () => {
  describe("when all the legs are priced", () => {
    beforeAll(async () => {
      ...
      await mockService.mockHttpRequest(getObbImply(thirdImplyBetsResponse));
      ...
      await browser.waitUntilDisplayed(firstPvpCardPO.playerContainer);

      // adding a selection from the first pvp card
      await firstPvpCardFirstBetButton.element.click();
      await browser.waitUntilDisplayed(placeButtonPO.label);

      // closing betslip and adding a selection in the second pvp card
      await betslipDrawerPO.header.click();
      await secondPvpCardSecondBetButton.element.scrollIntoView({ block: "center" });
      await secondPvpCardSecondBetButton.element.click();

      // adding a selection in the third pvp card
      await thirdPvpCardSecondBetButton.element.scrollIntoView({ block: "center" });
      await thirdPvpCardSecondBetButton.element.click();

      // expanding betslip
      await minimizedPO.title.click();
      await browser.waitUntilDisplayed(sliderPO.element);
    });

    it("[PRPI-6966]then the settlement condition card is displayed with 3 of 3", async () => {
      expect(await settlementConditionCardPO.label.getText()).toBe("3 of 3");
    });

    it("[PRPI-6967] odds are 8.5", async () => {
      expect(await oddsInputFieldPO.numberField.getValue()).toBe("8.5");
    });

    describe("and when I change the slider position", () => {
      beforeAll(async () => {
        await sliderMinusButtonPO.click();
        await sliderMinusButtonPO.click();
        await browser.waitUntil(async () => (await settlementConditionCardPO.label.getText()) === "1 of 3");
      });

      it("[PRPI-6968] the settlement condition card is displayed with be 1 of 3", async () => {
        expect(await settlementConditionCardPO.label.getText()).toBe("1 of 3");
      });

      it("[PRPI-6969] odds are updated to 6.5", async () => {
        expect(await oddsInputFieldPO.numberField.getValue()).toBe("6.5");
      });
      ...
      describe("and when I enter stake for the multiple", () => {
        ...
        it("[PRPI-6970] returns should be updated for the multiple bet", async () => {
          expect(await multipleReturnValuesPO.pnl.getText()).toBe("$143.00");
        ...
        describe("and when I place the multiple bet", () => {
          ...
          it("[PRPI-6971] receipt shows selections to win 1 of 3", async () => {
            expect(await settlementConditionCardPO.label.getText()).toBe("1 of 3");
          });
        });
      });
  ...
  describe("when at least one of the legs is not priced", () => {
    beforeAll(async () => {
      await mockService.mockHttpRequest(await getIndexHTML(BFF_MOCK.urn));
      const eventLayout = getEventLayout(BFF_MOCK);
      await mockService.mockHttpRequest(eventLayout);
      ...
      await mockService.mockHttpRequest(getObbImply(firstImplyBetsResponse));
      await mockService.mockHttpRequest(getObbImply(secondImplyBetsResponse));
      await mockService.mockHttpRequest(getObbImply(thirdImplyBetsResponseNoPrice)); // price=null for x=1
    ...
    it("[PRPI-6972] then the settlement condition card is not displayed", async () => {
      expect(await settlementConditionCardPO.element.isDisplayed()).toBe(false);
    });
  });
});

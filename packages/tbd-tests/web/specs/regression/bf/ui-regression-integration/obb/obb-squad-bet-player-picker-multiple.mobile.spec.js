...
const secondBetButton = new SportsbookBetButtonPO(bettingButtons[1]);
      ...
      legs: [
        {
          ...
          betDefinitions: ["dee836678ab11930", "4d7d44cc9abf3dd4"],
          result: {
            errorDetails: null,
            resultCode: "SUCCESS",
            __typename: "ObbResult",
...
describe("OBB - Squad Bet - Player Picker", () => {
  describe("Given I'm on an event page", () => {
    ...
    describe("And I have a squadbet card", () => {
      ...
      describe("When I click on two bet buttons", () => {
        beforeAll(async () => {
          await betButton.element.click();
          await secondBetButton.element.click();
        });

        it("[PRPI-5204]And the bet buttons change to selected state", async () => {
      ...
      describe("When I click on 'Add to betslip' button", () => {
        ...
        it("[PRPI-5205]Then the bottom sheet is closed", async () => {
          await browser.waitUntilDisplayed(multiplesCardPO.header);

          expect(await obbSquadBetPlayerPickerPO.element.isDisplayed()).toBeFalsy();
        });

        it("[PRPI-5206]And the bets are added to betslip with the selected players", async () => {
          const multiplesText = await multiplesCardPO.element.getText();

          expect(multiplesText).toContain("Myles Hippolyte & Sam Hutchinson");

          expect(multiplesText).toContain("1+ Shots On Target Between Them");

          expect(multiplesText).toContain("2+ Shots On Target Between Them");
        });
      });
    });
  });

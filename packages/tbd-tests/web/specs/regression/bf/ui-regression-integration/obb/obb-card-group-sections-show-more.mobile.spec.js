const { ObbCardGroupPO, ObbSectionPO } = require("../../../../../page-objects");
...
describe("OBB - Card Group - Sections - Show More", () => {
  beforeAll(async () => {
  ...
  describe("When a stacked layout has 2 cards and a limit of 1 card", () => {
    it("[PRPI-7004] Then I should see 1 card", async () => {
      await browser.waitUntil(async () => (await obbSectionPO.cards.length) === 1);

      expect(await obbSectionPO.cards.length).toBe(1);
    });

    describe("When I tap on the show more button", () => {
      beforeAll(async () => {
        await obbSectionPO.showMoreButton.click();
        await browser.waitUntil(async () => (await obbSectionPO.cards.length) === 2);
      });

      it("[PRPI-7005] Then I should see 2 cards", async () => {
        expect(await obbSectionPO.cards.length).toBe(2);
      });

      describe("And I tap on the show less button", () => {
        beforeAll(async () => {
          await obbSectionPO.showMoreButton.scrollIntoView();
          await obbSectionPO.showMoreButton.waitForClickable();
          await obbSectionPO.showMoreButton.click();
          await browser.waitUntil(async () => (await obbSectionPO.cards.length) === 1);
        });

        it("[PRPI-7006] Then I should see 1 card", async () => {
          expect(await obbSectionPO.cards.length).toBe(1);
        });
      });
    });

...
const secondEditSquadLinkPO = new ActionLinkPO(obbSquadVsSquadCardPO.secondEditSquadLink);

const obbSquadVsSquadPlayerPickerPO = new ObbSquadVsSquadPlayerPickerPO();
const playerPickerFirstButtonPO = new SportsbookBetButtonPO(obbSquadVsSquadPlayerPickerPO.firstBetButton);
const playerPickerSecondButtonPO = new SportsbookBetButtonPO(obbSquadVsSquadPlayerPickerPO.secondBetButton);
...
describe("OBB - Squad vs Squad", () => {
  describe("Event page", () => {
    beforeAll(async () => {
      await mockService.mockHttpRequest(
        await getIndexHTML(BFF_MOCK.urn, {
          experiments: [
            {
              name: "sbg-cms-obb-squadvssquad-player-picker-consistency",
              variant: "squad-vs-squad-player-picker-consistency-variant",
            },
          ],
        }),
      );
    ...
    describe("And I have a squad vs squad card", () => {
      ...
      it("[PRPI-8334]should have the correct players selected", async () => {
      ...
      it("[PRPI-8335]should display the bet buttons with the correct odds", async () => {
      ...
      describe("When I click on Edit Squad 1 link", () => {
        ...
        it("[PRPI-8336]should display the player picker modal", async () => {
          expect(await obbSquadVsSquadPlayerPickerPO.element.isDisplayed()).toBe(true);
        });

        it("[PRPI-8337]display a list of players", async () => {
        ...
        it("[PRPI-8338]should highlight the selected players for Squad 1", async () => {
        ...
        it("[PRPI-8339]should disabled the selected players for Squad 2", async () => {
      ...
      describe("When I click on a unselected player", () => {
        ...
        it("[PRPI-8340]should select the player and update the quote", async () => {
          ...
          expect(await playerPickerFirstButtonPO.odd.getText()).toBe("3.01");
        });
      });

      describe("When I click on the first bet button", () => {
        beforeAll(async () => {
          await playerPickerFirstButtonPO.element.click();
          await browser.waitUntilDisplayed(betDetailsPO.element);
        });

        it("[PRPI-8341]should add the selection to the betslip", async () => {
          expect(await betDetailsPO.title.getText()).toBe("Cole Palmer, Nicolas Jackson & Radamel Falcao");
        });
      });

      describe("When I close the betslip", () => {
        beforeAll(async () => {
          await betslipDrawerPO.header.click();
          await browser.waitUntilDisplayed(obbSquadVsSquadCardPO.element);
          await browser.waitUntilDisplayed(obbSquadVsSquadCardPO.firstBetButton);
        });
        it("[PRPI-8342]should see the SquadVsSquad card with the first bet button odd updated and selected", async () => {
          expect(await firstButtonPO.odd.getText()).toBe("3.01");
          expect(await browser.containsClass(firstButtonPO.element, SportsbookBetButtonPO.states.selected)).toBe(true);
        });
      });

      describe("When I click on the first bet button of the SquadVsSquad card", () => {
        beforeAll(async () => {
          await firstButtonPO.element.click();
        });

        it("[PRPI-8343]should see the bet button not selected", async () => {
          expect(await browser.containsClass(firstButtonPO.element, SportsbookBetButtonPO.states.selected)).toBe(false);
        });
      });

      describe("When I click on Edit Squad 2 link", () => {
        beforeAll(async () => {
          await secondEditSquadLinkPO.element.click();
          await mockService.mockHttpRequest(getObbEventParticipants(eventParticipants));
          await browser.waitUntilDisplayed(obbSquadVsSquadPlayerPickerPO.element);
        });

        it("[PRPI-8344]should display the player picker modal", async () => {
          expect(await obbSquadVsSquadPlayerPickerPO.element.isDisplayed()).toBe(true);
        });

        it("[PRPI-8345]display a list of players", async () => {
          expect(await obbSquadVsSquadPlayerPickerPO.playersList.isDisplayed()).toBe(true);
          expect(await obbSquadVsSquadPlayerPickerPO.playersRows.length).toBe(6);
        });
      });

      describe("When I click on the second bet button", () => {
        beforeAll(async () => {
          await playerPickerSecondButtonPO.element.click();
          await browser.waitUntilDisplayed(betDetailsPO.element);
        });

        it("[PRPI-8346]should add the selection to the betslip", async () => {
          expect(await betDetailsPO.title.getText()).toBe("Osmande Diomande & Luis Suarez");
        });
      });
      ...
      describe("When I close the betslip", () => {
        ...
        it("[PRPI-8636]should see the SquadVsSquad card with the second bet button odd updated and selected", async () => {
          expect(await secondBetButtonPO.odd.getText()).toBe("1.5");
          expect(await browser.containsClass(secondBetButtonPO.element, SportsbookBetButtonPO.states.selected)).toBe(
            true,
          );
        });
      });

      describe("When I click on the Squad 2 bet button", () => {
        beforeAll(async () => {
          await secondBetButtonPO.element.click();
        });

        it("[PRPI-8347]should see the bet button not selected", async () => {
          expect(await browser.containsClass(secondBetButtonPO.element, SportsbookBetButtonPO.states.selected)).toBe(
            false,
          );
        });
      ...
      describe("When I click on the Squad 1 bet button", () => {
        ...
        it("[PRPI-8348]should add the selection to the betslip", async () => {
          expect(await betDetailsPO.title.getText()).toBe("Cole Palmer, Nicolas Jackson & Radamel Falcao");
        });
      });
      ...
      describe("When I enter a stake and place bet", () => {
        ...
        it("[PRPI-8349]then the bet is played successfully", async () => {
          expect(await receiptTitle.getText()).toContain("Bet Placed");
        });

        it("[PRPI-8350]and receipt is displayed", async () => {

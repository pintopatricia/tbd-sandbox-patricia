const { ObbPvpCardPO, ActionLinkPO, BottomSheetPO, RichTextPO, QuickLinkPO } = require("../../../../../page-objects");
...
const moreInfoLinkPO = new ActionLinkPO();
...
describe("OBB - Card Group - More Info", () => {
  beforeAll(async () => {
  ...
  describe("When the OBB Card Group is loaded with the 'More Info' link", () => {
    it("[PRPI-6980] should display the 'More Info' link", async () => {
      expect(await moreInfoLinkPO.element.getText()).toBe("More Info");
    });

    describe("When I click on the 'More Info'", () => {
      beforeAll(async () => {
        await moreInfoLinkPO.element.click();
        await browser.waitUntilDisplayed(bottomSheetPO.element, "Bottom Sheet not displayed");
      });

      it("[PRPI-6981] should display the 'More Info' bottom sheet with the correct title", async () => {
        expect(await bottomSheetPO.headerTitle.getText()).toBe("More Info");
      });

      it("[PRPI-6982] should display the correct rich text content", async () => {
        expect(await richTextPO.getParagrphsList[0].getText()).toBe("Rich content paragraph");
      });

      it("[PRPI-6983] should display the quicklink", async () => {
        expect(await quickLinkPO.element.isDisplayed()).toBe(true);
        expect(await quickLinkPO.title.getText()).toBe("Some url");
      });

      describe("When I click on the quicklink", () => {
        beforeAll(async () => {
          await quickLinkPO.element.click();
          await browser.waitUntilBrowserUrlContains("https://www.betfair.com/betting");
        });

        it("[PRPI-6984] should navigate to the correct page", async () => {
          expect(await browser.getUrl()).toContain("www.betfair.com/betting");
        });
      });
    });
  });

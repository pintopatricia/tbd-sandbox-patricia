...
const secondSbkButton = new SportsbookBetButtonPO(inlineSportsbookMarketPO.betButtons[1]);

describe("User navigates", () => {
  describe("to the sbk sport page", () => {
    beforeAll(async () => {
      ...
      await browser.waitUntilEquals(secondSbkButton.odd, "-");
    });

    it("[PRPI-7183] The selections are displayed not clickable", async () => {
      expect(await inlineSportsbookMarketPO.betButtons[0].getText()).toEqual("-");
      expect(await inlineSportsbookMarketPO.betButtons[1].getText()).toEqual("-");
      expect(await inlineSportsbookMarketPO.betButtons[2].getText()).toEqual("-");
    });
  });

  describe("to the exc sport page", () => {
    beforeAll(async () => {
      ...
      await mockService.mockHttpRequest(getMarkets(ERO_MOCK_SUSPENDED));
      await browser.url(routes.getSportViewUrl(EVENT_TYPE_ID));
      await browser.waitUntilDisplayed(sportPagePO.actionLink[0]);
      await browser.waitUntilEquals(secondBackButton.odd, "2.1");
    });

    it("[PRPI-7184] The back selections are displayed not clickable", async () => {
    ...
    it("[PRPI-7185] The lay selections are displayed not clickable", async () => {

...
const ewOptionPO = new OptionPO(multiplesCardPO.element);
...
describe("SBK Free Bets Wallets - E/W Multiples", () => {
  beforeAll(async () => {
    ...
    // Open DOUBLE Generosity Wallets
  ...
  it("[PRPI-7858] should display a generosity alert on double with the correct amount of free bets and a remove action", async () => {
    expect(await doubleFreeBetsWalletsAlertPO.element.isDisplayed()).toBe(true);
    expect(await doubleFreeBetsWalletsAlertPO.message.getText()).toBe("Includes 2 x $1.00 in Free Bets");
    expect(await doubleFreeBetsWalletsAlertPO.actionLink.getText()).toBe("Remove");
  });

  it("[PRPI-7859] should display a generosity alert on place footer with the correct amount of free bets and a remove action", async () => {

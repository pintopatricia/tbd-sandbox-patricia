...
const oneLineMultipleControlsPO = new BetControlsPO(sportsbookPlacePanelPO.collapsableSections[0]);
...
const trebbleMultipleOptionPO = new OptionPO(oneLineMultipleControlsPO.element);
const trixieMultipleOptionPO = new OptionPO(multiLinesMultiplesPO.element);
const trebbleMultipleStakeInputFieldPO = new CurrencyNumberInputFieldPO(oneLineMultipleControlsPO.currencyInput);
const trixieMultipleStakeInputFieldPO = new CurrencyNumberInputFieldPO(firstMultipleControlsPO.currencyInput);
const placeButtonPO = new PrimaryButtonPO();

const sportsbookReceiptPanelPO = new SportsbookReceiptPanelPO();
const trebleSummaryPO = new BetSummaryPO(sportsbookReceiptPanelPO.multiples[0]);
const trixieSummaryPO = new BetSummaryPO(sportsbookReceiptPanelPO.multiples[1]);
...
describe("Betslip - SBK Single Bet Placement", () => {
  ...
  describe("when user adds three selections", () => {
    ...
    it("[PRPI-7997] Should see 'ADDITIONAL MULTIPLES' title visible on the accordion", async () => {
      const cardPO = new CardPO(betslipDrawerPO.cards[1]);

      expect(await cardPO.title.getText()).toBe("ADDITIONAL MULTIPLES");
    });

    it("[PRPI-7998] And I should see 2 bet types multi-combination available: Treble and Trixie (x4)", async () => {
      expect(await oneLineMultiplePO.text.getText()).toBe("Treble");
      expect(await firstMultipleControlsPO.lines.getText()).toBe("Trixie (x4)");
    });

    it("[PRPI-7999] And I should see 'Each Way' edge visible for Trixie (x4)", async () => {
      expect(await firstMultipleControlsPO.eachWay.isDisplayed()).toBe(true);
    });

    it("[PRPI-8000] And I should see the 'Returns' label and value '0.00' visible", async () => {
    ...
    describe("when setting a 2 stake for treble with EW", () => {
      beforeAll(async () => {
        await trebbleMultipleOptionPO.input.click();
        await browser.waitUntil(
          async () => {
            const isSelected = await trebbleMultipleOptionPO.input.isSelected();

            return isSelected;
          },
          {
            timeoutMsg: "Option was not selected",
          },
        );

        await trebbleMultipleStakeInputFieldPO.element.scrollIntoView({ block: "center" });
        await browser.waitUntilDisplayed(trebbleMultipleStakeInputFieldPO.element);

        await trebbleMultipleStakeInputFieldPO.numberField.waitForClickable();
        await trebbleMultipleStakeInputFieldPO.numberField.click();
        await trebbleMultipleStakeInputFieldPO.setValue("2");

        await browser.waitUntilEquals(trebbleMultipleStakeInputFieldPO.numberField, "2");
      });

      it("[PRPI-8001] should have checkbox selected", async () => {
        expect(await trebbleMultipleOptionPO.input.isSelected()).toBe(true);
      });

      it("[PRPI-8002] should display multiplier on the left side of the size input field", async () => {
        expect(await trebbleMultipleStakeInputFieldPO.multiplier.getText()).toContain("2x");
      });

      it("[PRPI-8003] should display 'returns' label under the EW component", async () => {
        expect(await oneLineMultipleControlsPO.returns.isDisplayed()).toBe(true);
      });

      describe("when bet type 'Trixie' is selected", () => {
        beforeAll(async () => {
          await trixieMultipleOptionPO.input.click();
          await browser.waitUntil(
            async () => {
              const isSelected = await trixieMultipleOptionPO.input.isSelected();
          ...
          await trixieMultipleStakeInputFieldPO.element.waitForClickable();
          await trixieMultipleStakeInputFieldPO.element.click();
          await trixieMultipleStakeInputFieldPO.setValue("2");
          await browser.waitUntilEquals(trixieMultipleStakeInputFieldPO.numberField, "2");
        });

        it("[PRPI-8004] should have checkbox selected", async () => {
          expect(await trixieMultipleOptionPO.input.isSelected()).toBe(true);
        });

        it("[PRPI-8005] should display multiplier on the left side of the size input field", async () => {
          expect(await trixieMultipleStakeInputFieldPO.multiplier.getText()).toContain("2x");
        });

        it("[PRPI-8006] should display 'returns' label under the EW component", async () => {
          expect(await firstMultipleControlsPO.returns.isDisplayed()).toBe(true);
        });

        describe("when unselected checkbox of EW component in bet type 'Trebble'", () => {
          beforeAll(async () => {
            await trebbleMultipleOptionPO.input.click();
            await browser.waitUntil(async () => !(await trebbleMultipleOptionPO.input.isSelected()), {
              timeoutMsg: "Option was not selected",
            });
          });

          it("[PRPI-8007] should have checkbox unselected", async () => {
            expect(await trebbleMultipleOptionPO.input.isSelected()).toBe(false);
          });

          it("[PRPI-8008] should not display multiplier on the left side of the size input field", async () => {
            expect(await trebbleMultipleStakeInputFieldPO.multiplier.isDisplayed()).toBe(false);
          });

          describe("when select EW checkbox in 'Treble' again", () => {
            beforeAll(async () => {
              await trebbleMultipleOptionPO.input.click();
              await browser.waitUntil(
                async () => {
            ...
            it("[PRPI-8009] should have checkbox selected", async () => {
              expect(await trebbleMultipleOptionPO.input.isSelected()).toBe(true);
            });

            it("[PRPI-8009] should display multiplier on the left side of the size input field", async () => {
              expect(await trebbleMultipleStakeInputFieldPO.multiplier.getText()).toContain("2x");
            });

            it("[PRPI-8009] should display 'returns' label under the EW component", async () => {
              expect(await oneLineMultipleControlsPO.returns.isDisplayed()).toBe(true);
            });

            describe("when pressing place bet button", () => {
              beforeAll(async () => {
                await mockService.mockHttpRequest(getPlaceBet(PLACE_SUCCESS));
                await browser.waitUntilDisplayed(placeButtonPO.element);
                await placeButtonPO.element.click();
                await browser.waitUntilDisplayed(sportsbookReceiptPanelPO.element);
              });

              it("[PRPI-8009] should show receipt with 'Treble' bet info", async () => {
                expect(await trebleSummaryPO.title.getText()).toBe("Treble");
              });

              it("[PRPI-8009] should show receipt with 'Trixie' bet info", async () => {
                expect(await trixieSummaryPO.title.getText()).toBe("Trixie");
              });
            });
          });
        });

...
const sportsbookStakeInputField = new CurrencyNumberInputFieldPO(betControlsPO.currencyInput);
...
describe("Multiple Singles Confirm Experience", () => {
  describe("when one selection is added to betslip", () => {
    ...
    describe("and the user inserts a stake value above the wallet balance", () => {
      beforeAll(async () => {
        await sportsbookStakeInputField.numberField.waitForClickable();
        await sportsbookStakeInputField.numberField.setValue("150");
        await browser.waitUntilEquals(placeButtonPO.label, "Deposit to Place $150.00 Bet");
      });

      it("[PRPI-7779] should display 'Deposit to Place $150.00 Bet' on the place button", async () => {
        expect(await placeButtonPO.label.getText()).toEqual("Deposit to Place $150.00 Bet");
      });
      ...
      describe("when the user advances to confirm step", () => {
        ...
        it("[PRPI-7780] should display the confirm screen", async () => {
          expect(await sportsbookConfirmPO.element.isDisplayedInViewport()).toBe(true);
        });

        it("[PRPI-7781] should display a double", async () => {
          expect(await betControlsPO.element.isDisplayedInViewport()).toBe(true);
          expect(await betControlsPO.betType.getText()).toEqual("Double");
        });

        it("[PRPI-7782] should display 'Deposit to Confirm $150.00 Bet' in the place button", async () => {
          expect(await placeButtonPO.label.getText()).toEqual("Deposit to Confirm $150.00 Bet");
        });
      });
      ...
      describe("and all runners gets suspended", () => {
        beforeAll(async () => {
          ...
          await browser.waitUntilEquals(placeButtonPO.label, "Suspended");
        });

        it("[PRPI-7783] should display a disabled confirm button with a 'Suspended' label", async () => {
          expect(await placeButtonPO.label.getText()).toEqual("Suspended");
      ...
      describe("and the user clicks the edit button", () => {
        beforeAll(async () => {
          ...
          await browser.waitUntilEquals(placeButtonPO.label, "Please Enter Stake");
          await editButtonPO.element.waitForClickable();
          await editButtonPO.element.click();
        });

        it("[PRPI-7784] should return to the multiples experience", async () => {
          expect(await sportsbookPlacePanelPO.element.isDisplayedInViewport()).toBe(true);
        });
        ...
        describe("when the user adds a stake to the only available bet", () => {
          ...
          describe("when the user advances to confirm step", () => {
            ...
            it("[PRPI-7785] should display the confirm screen", async () => {
              expect(await sportsbookConfirmPO.element.isDisplayedInViewport()).toBe(true);
            });

            it("[PRPI-7785] should display a single", async () => {
              expect(await betControlsPO.element.isDisplayedInViewport()).toBe(true);

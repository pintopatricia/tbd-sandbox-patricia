...
const doubleGenerosityWalletsAlertPO = new AlertPO(multiplesCardPO.element.$(ALERT_TEST_ID));

const singlesCardContentPO = new SinglesCardPO(singlesCardPO.content);
...
const firstSingleGenerosityWalletsAlertPO = new AlertPO(firstSinglePO.element.$(ALERT_TEST_ID));
...
const secondSingleGenerosityWalletsAlertPO = new AlertPO(secondSinglePO.element.$(ALERT_TEST_ID));
...
describe("SBK Generosity Wallet - Confirm Multiples", () => {
  beforeAll(async () => {
  ...
  describe("on singles", () => {
    afterAll(async () => {
      await singlesCardPO.headerWrapper.waitForClickable();
      await singlesCardPO.headerWrapper.click();
      await browser.waitUntilNotDisplayed(singlesCardPO.content, "SINGLES section is not collapsed");
    });

    it("[PRPI-7848] should display the generosity alert on each single", async () => {
      expect(await firstSingleGenerosityWalletsAlertPO.element.isDisplayed()).toBe(true);
      expect(await firstSingleGenerosityWalletsAlertPO.message.getText()).toBe("Includes $3.00 in Free Bets");
      expect(await secondSingleGenerosityWalletsAlertPO.element.isDisplayed()).toBe(true);
      expect(await secondSingleGenerosityWalletsAlertPO.message.getText()).toBe("Includes $5.00 in Free Bets");
    });

    it("[PRPI-7849] should not display the generosity alert remove action", async () => {
      expect(await firstSingleGenerosityWalletsAlertPO.actionLink.isDisplayed()).toBe(false);
      expect(await secondSingleGenerosityWalletsAlertPO.actionLink.isDisplayed()).toBe(false);
    });
  });

  describe("on multiples", () => {
    beforeAll(async () => {
      await multiplesCardPO.element.click();
      await browser.waitUntilDisplayed(multiplesCardPO.content, "BET BUILDER section is not expanded");
    });

    it("[PRPI-7850] should display the generosity alert on multiples", async () => {
      expect(await doubleGenerosityWalletsAlertPO.element.isDisplayed()).toBe(true);
      expect(await doubleGenerosityWalletsAlertPO.message.getText()).toBe("Includes $2.00 in Free Bets");
    });

    it("[PRPI-7851] should not display the generosity alert remove action", async () => {
      expect(await doubleGenerosityWalletsAlertPO.actionLink.isDisplayed()).toBe(false);
    });
  });

  describe("on the place footer", () => {
    it("[PRPI-7852] should display the generosity alert on each single", async () => {
    ...
    it("[PRPI-7853] should not display the generosity alert remove action", async () => {
      expect(await freeBetsWalletsAlertPO.actionLink.isDisplayed()).toBe(false);
    });

    it("[PRPI-7854] should display the confirm button with 'Confirm $10.00 Selection' message", async () => {
      expect(await confirmButtonPO.element.isDisplayed()).toBe(true);
      expect(await confirmButtonPO.label.getText()).toBe("Confirm $10.00 Selection");
    });
  });
});

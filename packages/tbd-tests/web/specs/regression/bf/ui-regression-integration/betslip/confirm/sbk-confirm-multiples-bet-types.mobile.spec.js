...
const secondMultiLineStakeInputField = new CurrencyNumberInputFieldPO(secondMultipleControlsPO.currencyInput);
...
describe("Multiples Bet Confirm Experience", () => {
  describe("when adding multiple selections to betslip", () => {
    ...
    describe("and the user inserts a stake value for treble", () => {
      beforeAll(async () => {
      ...
      describe("when the user advances to confirm step", () => {
        beforeAll(async () => {
          ...
          await browser.waitUntilDisplayed(
            oneLineMultipleBetLegsPO.element,
            "One line multiple is not visible in confirm screen",
          );
        });

        it("[PRPI-7789] should display the confirm screen", async () => {
          expect(await sportsbookConfirmPO.element.isDisplayedInViewport()).toBe(true);
        });

        it("[PRPI-7790] should display a treble", async () => {
          expect(await betControlsPO.element.isDisplayedInViewport()).toBe(true);
          expect(await oneLineMultipleControlsPO.lines.getText()).toBe("Treble (x4)");
        });

        it("[PRPI-7791] should display one line multiples bet legs", async () => {
      ...
      describe("and one of the runners gets suspended", () => {
        ...
        it("[PRPI-7792] should disable the place button", async () => {
      ...
      describe("and the user clicks the edit button", () => {
        beforeAll(async () => {
          ...
          await browser.waitUntilEquals(placeNotification.message, "Market Suspended");
        });

        it("[PRPI-7793] should return to the multiples experience with market suspended notification", async () => {
          expect(await sportsbookPlacePanelPO.element.isDisplayedInViewport()).toBe(true);
        ...
        describe("when the user adds a stake for another available bet", () => {
          beforeAll(async () => {
            await secondMultiLineStakeInputField.numberField.waitForClickable();
          ...
          describe("and the user advances to confirm step", () => {
            beforeAll(async () => {
              ...
              await browser.waitUntilDisplayed(
                oneLineMultipleBetLegsPO.element,
                "One line bet is not visible in confirm screen",
              );
            });

            it("[PRPI-7794] should display the confirm screen", async () => {
              expect(await sportsbookConfirmPO.element.isDisplayedInViewport()).toBe(true);
            });

            it("[PRPI-7794] should display only a trixie", async () => {
              expect(await betControlsPO.element.isDisplayedInViewport()).toBe(true);
              expect(await oneLineMultipleControlsPO.lines.getText()).toBe("Trixie (x4)");
            });

            it("[PRPI-7794] should display multi line multiples bet legs", async () => {
              expect(await oneLineMultipleBetLegsPO.element.isDisplayed()).toBe(true);
              expect(await oneLineMultipleBetLegsPO.selections.length).toBe(3);
              expect(await sportsbookPlacePanelCardPO.title.getText()).toBe("MULTIPLES");
            });
          });
          ...
          describe("when the market opens again", () => {
            ...
            it("[PRPI-7795] should disable confirm button and display availability notification", async () => {
    ...
    describe("when the user adds an one line multiple bet", () => {
      beforeAll(async () => {
        await editButtonPO.element.waitForClickable();
        await editButtonPO.element.click();

        await oneLineMultipleStakeInputField.element.waitForClickable();
        await oneLineMultipleStakeInputField.element.scrollIntoView();
        await oneLineMultipleStakeInputField.element.click();
        await oneLineMultipleStakeInputField.setValue("0.12");
      });
      ...
      describe("and advances to confirm step", () => {
        beforeAll(async () => {
          ...
          await browser.waitUntilDisplayed(
            oneLineMultipleBetLegsPO.element,
            "One line multiple bet legs is not visible in confirm screen",
          );
        });

        it("[PRPI-7796] should not display multi line multiples bet legs", async () => {
          expect(await multiLinesMultiplesBetLegsPO.element.isDisplayed()).toBe(false);
        });

        it("[PRPI-7797] should display one line multiple bet legs", async () => {
          expect(await oneLineMultipleBetLegsPO.element.isDisplayed()).toBe(true);
          expect(await oneLineMultipleBetLegsPO.selections.length).toBe(4);
          expect(await sportsbookPlacePanelCardPO.title.getText()).toBe("MULTIPLES");
        });

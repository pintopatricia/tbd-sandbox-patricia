...
describe("Bet Builder Failure", () => {
  describe("when the user adds two combinable selections from the same event and opens the betslip", () => {
    beforeAll(async () => {
      ...
      await browser.waitUntilEquals(firstOddsPO.numberField, "N/A", "Failed bet builder was not displayed");
      await browser.waitUntilDisplayed(firstAlertPO.element, "Notification is not yet displayed");
    });

    it("[PRPI-7682] The message 'This Bet Builder needs one more selection to be completed' should be displayed", async () => {
      expect(await firstAlertPO.message.getText()).toEqual("This Bet Builder needs one more selection to be completed");
    });
    ...
    describe("When adding another combinable selection from the same event", () => {
      beforeAll(async () => {
        ...
        await sportsbookMinimizedPO.element.waitForClickable();
        await sportsbookMinimizedPO.element.click();
        await browser.waitUntilEquals(firstOddsPO.numberField, "3.2", "Valid bet builder was not displayed");
      });

      it("[PRPI-7683] The message 'This Bet Builder needs one more selection to be completed' should NOT be displayed", async () => {
        expect(await alertsPO.element.isExisting()).toBe(false);
      });
    });
  });
});

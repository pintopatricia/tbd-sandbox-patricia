...
describe("Sportsbook Bet Placement", () => {
  describe("with the format being American and the accept odds movement off", () => {
    beforeAll(async () => {
      await mockService.mockHttpRequest(
        await getIndexHTML(BFF_MOCK.urn, { oddsMovement: "false", sportsbookOddsDisplay: "AMERICAN" }),
      );
      ...
      await browser.waitUntil(
        AppPO.sportsbookRunnerBetButtonHasPrice({ market: eventPagePO.markets[0], price: -1000 }),
      );
    ...
    it("[PRPI-8076] the sbk place panel should display american odds", async () => {
      expect(await oddsInputField.numberField.getValue()).toBe("-1000");
    });
    ...
    describe("and placing the bet", () => {
      ...
      it("[PRPI-8077] should show the receipt title", async () => {
        expect(await receiptTitle.getText()).toBe("Bet Placed");
      });

      it("[PRPI-8078] should show receipt title", async () => {
        expect(await receiptTitle.isDisplayed()).toBe(true);
      });

      it("[PRPI-8079] should show the placed selection information", async () => {
      ...
      it("[PRPI-8080] should show the placed selection bet details", async () => {
        expect(await firstSingleSegmentsPO.leftValue.getText()).toBe("-1000");
  ...
  describe("when placing a bet with failure", () => {
    beforeAll(async () => {
      ...
      await browser.waitUntilDisplayed(placePanelPO.element, "Place panel not displayed after bet button click", 500);

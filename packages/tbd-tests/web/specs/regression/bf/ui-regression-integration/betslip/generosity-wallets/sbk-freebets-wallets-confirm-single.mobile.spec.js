...
const generosityWalletsAlertPO = new AlertPO(singlesCardPO.element);
...
describe("SBK Free Bets Wallets - Singles Confirm", () => {
  ...
  it("[PRPI-7855] should display a disabled generosity button", async () => {
    expect(await betControlsPO.generosityWalletButton.isDisplayed()).toBe(true);
    expect(await betControlsPO.generosityWalletButton.isEnabled()).toBe(false);
  });

  it("[PRPI-7856] should display on confirm step a generosity alert with 'Includes $5.00 in Free Bets' message without any action available", async () => {
    expect(await generosityWalletsAlertPO.element.isDisplayed()).toBe(true);
    expect(await generosityWalletsAlertPO.actionLink.isDisplayed()).toBe(false);
    expect(await generosityWalletsAlertPO.message.getText()).toBe("Includes $5.00 in Free Bets");
  });

  it("[PRPI-7857] should display the confirm button with 'Confirm $5.00 Selection' message", async () => {
    expect(await confirmButtonPO.element.isDisplayed()).toBe(true);
    expect(await confirmButtonPO.label.getText()).toBe("Confirm $5.00 Selection");
  });
});

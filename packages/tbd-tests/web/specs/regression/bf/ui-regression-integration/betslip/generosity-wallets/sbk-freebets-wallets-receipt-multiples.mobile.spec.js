...
const firstSingleFreeBetsWalletsAlertPO = new AlertPO(sportsbookReceiptPanelPO.singles[0]);
const secondSingleFreeBetsWalletsAlertPO = new AlertPO(sportsbookReceiptPanelPO.singles[1]);
...
describe("SBK Free Bets Wallets - Multiples Receipt", () => {
  ...
  it("[PRPI-7876] should display for multiples the generosity alert with 'Includes $2.00 in Free Bets' message without any action available", async () => {
    ...
    expect(await multipleFreeBetsWalletsAlertPO.message.getText()).toBe("Includes $2.00 in Free Bets");
  });

  it("[PRPI-7877] should display for the first single the generosity alert with 'Includes $3.00 in Free Bets' message without any action available", async () => {
    expect(await firstSingleFreeBetsWalletsAlertPO.element.isDisplayed()).toBe(true);
    expect(await firstSingleFreeBetsWalletsAlertPO.actionLink.isDisplayed()).toBe(false);
    expect(await firstSingleFreeBetsWalletsAlertPO.message.getText()).toBe("Includes $3.00 in Free Bets");
  });

  it("[PRPI-7878] should display for the second single the generosity alert with 'Includes $5.00 in Free Bets' message without any action available", async () => {
    expect(await secondSingleFreeBetsWalletsAlertPO.element.isDisplayed()).toBe(true);
    expect(await secondSingleFreeBetsWalletsAlertPO.actionLink.isDisplayed()).toBe(false);
    expect(await secondSingleFreeBetsWalletsAlertPO.message.getText()).toBe("Includes $5.00 in Free Bets");
  });
});

...
describe("Betslip - Exchange Successful Deposit Overlay", () => {
  ...
  describe("When the user has insufficient amount to place a bet and presses 'Deposit to Place Bet'", () => {
    beforeAll(async () => {
      ...
      await browser.waitUntilDisplayed(exchangeInlineConfirmPanelPO.element, "Confirm screen was not displayed");

      await exchangeInlineConfirmPanelPO.confirm.waitForClickable();
      await exchangeInlineConfirmPanelPO.confirm.click();

      await browser.waitUntilEquals(exchangeInlineConfirmPanelPO.confirm, "Deposit to Place Bet");

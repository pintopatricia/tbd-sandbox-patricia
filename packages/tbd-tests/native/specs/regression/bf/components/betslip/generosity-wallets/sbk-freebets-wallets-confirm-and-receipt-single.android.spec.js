...
const freeBetsWalletsAlertSO = new AlertSO(placeFooterSO.freeBetsWalletsAlert);
...
const freeBetsWalletsAlertReceiptSO = new AlertSO(betSportsbookReceiptSO.element);
...
describe("SBK Generosity Wallet - Singles Confirm and Receipt", () => {
  beforeAll(async () => {
  ...
  it("[PRPI-4158] should display a disabled generosity button", async () => {
    expect(await betControlsSO.generosityWalletButton.isDisplayed()).toBe(true);
    expect(await betControlsSO.generosityWalletButton.isEnabled()).toBe(false);
  });

  it("[PRPI-4159] should display the confirm button with 'Confirm $5.00 Selection' on confirm step", async () => {
    expect(await placeButtonSO.label.getText()).toBe("Confirm $5.00 Selection");
  });

  it("[PRPI-4160] should display on confirm step a generosity alert with 'Includes $5.00 in Free Bets' message without any action available", async () => {
    expect(await freeBetsWalletsAlertSO.element.isDisplayed()).toBe(true);
    expect(await freeBetsWalletsAlertSO.actionLinkText.isDisplayed()).toBe(false);
    expect(await freeBetsWalletsAlertSO.message.getText()).toBe("Includes $5.00 in Free Bets");
  });
  ...
  describe("and then the user places the bet", () => {
    ...
    it("[PRPI-4161] should display on receipt a generosity alert with 'Includes $5.00 in Free Bets' message without any action available", async () => {
      expect(await freeBetsWalletsAlertReceiptSO.element.isDisplayed()).toBe(true);
      expect(await freeBetsWalletsAlertReceiptSO.actionLinkText.isDisplayed()).toBe(false);
      expect(await freeBetsWalletsAlertReceiptSO.message.getText()).toBe("Includes $5.00 in Free Bets");
    });
  });
});

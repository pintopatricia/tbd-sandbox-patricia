...
describe("Betreceipt Notifications - Throttle disabled", () => {
  beforeAll(async () => {
  ...
  describe("When the user places a bet on 2 events, one in Football the other in HR", () => {
    ...
    it("[PRPI-3274] The toggle should be displayed disabled", async () => {
    ...
    describe("When the user turns notifications on", () => {
      ...
      it("[PRPI-3275] The toggle should be displayed enabled", async () => {
        expect(await optionSO.toggle.isDisplayed()).toBe(true);
        expect(await switchSO.element.getAttribute("selected")).toBe("true");
      });

      it("[PRPI-3276] The toast message should be displayed", async () => {
      ...
      it("[PRPI-3277] Shouldn't show the notification unavailable icon in BetSummary", async () => {
      ...
      describe("When the user closes the bet receipt and places a bet again only in a HR event ", () => {
        ...
        it("[PRPI-3278] The toggle should be displayed enabled", async () => {
          expect(await optionSO.toggle.isDisplayed()).toBe(true);
          expect(await switchSO.element.getAttribute("selected")).toBe("true");
        });
      });

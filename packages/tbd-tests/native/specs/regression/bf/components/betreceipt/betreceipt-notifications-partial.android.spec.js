...
const { getNSSRegister, getNSSSubscribePartial } = require("@ppb/tbd-shared/mocks/nss/nss.controller");
...
async function togglePush(subscribeStatus) {
...
describe("Betreceipt Notifications", () => {
  beforeAll(async () => {
    ...
    await mockService.mockHttpRequest(getNSSSubscribePartial(NSS_MOCK));
  ...
  describe("When the user places a bet on 3 events, with partial notifications", () => {
    beforeAll(async () => {
      await placeDoubleBet();
      await browser.waitUntil(async () => (await switchSO.switch.getAttribute("selected")) === "false");
    });

    it("[PRPI-3264] The toggle should be displayed disabled", async () => {
    ...
    describe("When the user turns notifications on for 2 events, one with notifications supported and the other no", () => {
      beforeAll(async () => {
        await togglePush("true");
      });

      it("[PRPI-3265] The toggle should be displayed enabled", async () => {
        expect(await optionSO.toggle.isDisplayed()).toBe(true);
        expect(await switchSO.switch.getAttribute("selected")).toBe("true");
      });

      it("[PRPI-3266] The toast message should be displayed", async () => {
      ...
      describe("When the user closes the bet receipt and places a bet again on the same events", () => {
        ...
        it("[PRPI-3267] The toggle should be displayed enabled", async () => {
          expect(await optionSO.toggle.isDisplayed()).toBe(true);
          expect(await switchSO.switch.getAttribute("selected")).toBe("true");
        });
        ...
        describe("And when the user disables the push notifications", () => {
          ...
          it("[PRPI-3267] The toast message should be displayed", async () => {
            expect(await snackbarSO.icon.isDisplayed()).toBe(true);
          ...
          it("[PRPI-3267] The toggle should be disabled", async () => {
            expect(await switchSO.switch.getAttribute("selected")).toBe("false");
          });
        });
      });

...
describe("Layout Entity - RegulatoryData with images and session logged in", () => {
  describe("When a danish loggedIn user is on a given screen And RegulatoryData is retrieved", () => {
    beforeAll(async () => {
      ...
      await browser.waitUntilDisplayed(regulatoryHeaderSO.headerSessionItem);
    });

    it("[PRPI-2514] The 5 images should be displayed", async () => {
      expect(await regulatoryHeaderSO.headerImageItems.length).toBe(5);
    });

    it("[PRPI-2515] The session timer should be visible", async () => {

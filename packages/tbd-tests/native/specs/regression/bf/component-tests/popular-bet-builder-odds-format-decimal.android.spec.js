...
const placeBetBuildersCardSO = new BetBuildersCardSO();
const betLegsSO = new BetLegsSO(placeBetBuildersCardSO.element);
const sportsbookBetButtonSO = new SportsbookBetButtonSO(popularBetBuilderSO.popularBetBuilderBetButton);
const firstBetSelectionSO = new BetSelectionDetailsSO(betLegsSO.selections[0]);
const secondBetSelectionSO = new BetSelectionDetailsSO(betLegsSO.selections[1]);
const thirdBetSelectionSO = new BetSelectionDetailsSO(betLegsSO.selections[2]);
const fourthBetSelectionSO = new BetSelectionDetailsSO(betLegsSO.selections[3]);
const fifthBetSelectionSO = new BetSelectionDetailsSO(betLegsSO.selections[4]);
...
describe("PopularBetBuilder", () => {
  ...
  describe("When the user clicks on the bet button", () => {
    beforeAll(async () => {
      ...
      await browser.waitUntilDisplayed(placeBetBuildersCardSO.betBuilders[0]);
    });

    it("[PRPI-3977] the information that the popular was added to the betslip should be displayed", async () => {
      expect(await placeBetBuildersCardSO.betBuilders.length).toBe(1);
    });

    it("[PRPI-3978] the first selection should be the one displayed from popular bet builder", async () => {
      expect(await firstBetSelectionSO.title.getText()).toEqual("Sgt Carneiro");
      expect(await firstBetSelectionSO.subtitle.getText()).toEqual("Anytime Goalscorer - Popular v Bet Builder");
    });

    it("[PRPI-3979] the second selection should be the one displayed from popular bet builder", async () => {
      expect(await secondBetSelectionSO.title.getText()).toEqual("Alberto Silva");
      expect(await secondBetSelectionSO.subtitle.getText()).toEqual(
    ...
    it("[PRPI-3980] the third selection should be the one displayed from popular bet builder", async () => {
      expect(await thirdBetSelectionSO.title.getText()).toEqual("Third Runner");
      expect(await thirdBetSelectionSO.subtitle.getText()).toEqual("Third Market - Popular v Bet Builder");
    });

    it("[PRPI-3981] the fourth selection should be the one displayed from popular bet builder", async () => {
      expect(await fourthBetSelectionSO.title.getText()).toEqual("Fourth Runner");
      expect(await fourthBetSelectionSO.subtitle.getText()).toEqual("Fourth Market - Popular v Bet Builder");
    });

    it("[PRPI-3982] the fifth selection should be the one displayed from popular bet builder", async () => {
      expect(await fifthBetSelectionSO.title.getText()).toEqual("Fifth Runner");
      expect(await fifthBetSelectionSO.subtitle.getText()).toEqual("Fifth Market - Popular v Bet Builder");
    });

    describe("and then clicks on it again", () => {
      beforeAll(async () => {
        await betslipDrawerSO.header.click();
        await browser.waitUntilNotDisplayed(placeBetBuildersCardSO.element);

...
const { getGenericLayout, getAppContext, getMarkets } = require("@ppb/tbd-shared/mocks/bff/bff.controller");
...
const firstPromo = new PromoBannerSO(genericViewSO.items[0]);
const secondPromo = new PromoBannerSO(genericViewSO.items[1]);
const thridPromo = new PromoBannerSO(genericViewSO.items[2]);
...
describe("Promotions - When application has brandSetting ENABLE_CIP_BANNER enabled", () => {
  ...
  it("[PRPI-4138] should show 3 promo banners", async () => {
    expect(await genericViewSO.items.length).toBe(3);
  });

  it("[PRPI-4140] should show first promo banner with all information", async () => {
  ...
  it("[PRPI-4141] should show second promo banner with all information", async () => {
  ...
  it("[PRPI-4142] should show third banner with all information", async () => {
    expect(await thridPromo.title.getText()).toBe("TITLE");
    expect(await thridPromo.subTitle.getText()).toBe("Short Sub");
    expect(await thridPromo.statusLabel.getText()).toBe("Show More");
    expect(await thridPromo.summary.getText()).toBe("Short Summary Short Terms");
  });
  ...
  describe("When user clicks on bet button", () => {
    ...
    it("[PRPI-3579] The betslip should exist", async () => {
      expect(await betDetailsSO.element.isExisting()).toBe(true);
    });

    it("[PRPI-3580] The betslip should be opened with correct title", async () => {
      expect(await betDetailsSO.title.getText()).toBe("Some Runner Name");
    });

    it("[PRPI-3581] The betslip should be opened with correct subtitle", async () => {
      expect(await betDetailsSO.subtitle.getText()).toBe("Match Odds - OddsBoost");
    });

    it("[PRPI-3582] The betslip should be opened with correct oddsboost price", async () => {
      expect(await sportsbookPriceInputSO.numberField.getText()).toBe("1.3");
    });

    it("[PRPI-3583] The betslip should be opened with correct previous odd", async () => {

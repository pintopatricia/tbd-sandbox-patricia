...
const { ObbPvpCardSO, SnackbarSO } = require("../../../../screen-objects");
...
describe("OBB - Pvp Card Add to Betslip", () => {
  describe("Given I'm on OBB Pvp Card", () => {
    beforeAll(async () => {
      ...
      await browser.waitUntilImageEquals(`${MODULE_NAME}_[PRPI-4871]_should_render_card_correctly`);
    });

    it("[PRPI-4871]_should_render_card_correctly", async () => {
      expect(
        (await browser.compareScreen(`${MODULE_NAME}_[PRPI-4871]_should_render_card_correctly`)).misMatchPercentage,
      ).toEqual(0);
    });
  });
});

const { getEventLayout, getHomeLayoutWithViewLink, getObbImply } = require("@ppb/tbd-shared/mocks/bff/bff.controller");
...
const secondPvpBetButtonSO = new SportsbookBetButtonSO(obbPvpCardSO.addButtons[1]);

const sportsbookPlacePanelSO = new SportsbookPlacePanelSO();

const firstSingleSO = new ObbSingleSO(sportsbookPlacePanelSO.obbSingles[0]);
const firstBetDetailsSO = new BetDetailsSO(firstSingleSO.element);

const secondSingleSO = new ObbSingleSO(sportsbookPlacePanelSO.obbSingles[1]);
...
describe("OBB - Betslip", () => {
  describe("Given I have one obb pvp card", () => {
    beforeAll(async () => {
    ...
    describe("When I click on 'Add to betslip' button on both bet buttons", () => {
      beforeAll(async () => {
        await browser.waitUntilClickableNative(firstPvpBetButtonSO.element);
        await firstPvpBetButtonSO.element.click();

        await mockService.mockHttpRequest(getObbImply(secondImplyBetsResponse));
        await betslipDrawerSO.header.click();

        await browser.waitUntilClickableNative(secondPvpBetButtonSO.element);
        await secondPvpBetButtonSO.element.click();

        await browser.waitUntilClickableNative(minimizedSO.element);
        await minimizedSO.element.click();

        await browser.waitUntilEquals(firstBetDetailsSO.title, "Nicolas Jackson");
        await browser.waitUntilEquals(secondBetDetailsSO.title, "Cole Palmer");

        await browser.waitUntilImageEquals(
          `${MODULE_NAME}_[PRPI-4869]_should_display_the_betslip_with_both_legs_correctly`,
        );
      });

      it("[PRPI-4869]_should_display_the_betslip_with_both_legs_correctly", async () => {
        expect(
          (

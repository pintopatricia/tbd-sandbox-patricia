const { getEventLayout, getHomeLayoutWithViewLink, getObbQuotes } = require("@ppb/tbd-shared/mocks/bff/bff.controller");
const { getWallets } = require("@flutter-global/uki-channels-http-clients/mock-index").WALLET;

const { ObbPvpCardSO, ObbPlayerGridSO } = require("../../../../screen-objects");
...
const MODULE_NAME = "obb_pvp_card_players_grid";
...
describe("OBB - Pvp Card Players Grid", () => {
  describe("Given I'm on OBB Pvp Card", () => {
    beforeAll(async () => {
      ...
      await obbPvpCardSO.firstPlayerContainer.click();

      await browser.waitUntilDisplayed(obbPlayersGridSO.element);
      await browser.waitUntilImageEquals(`${MODULE_NAME}_[PRPI-4872]_should_render_players_grid_correctly`);
    });

    it("[PRPI-4872]_should_render_players_grid_correctly", async () => {
      expect(
        (await browser.compareScreen(`${MODULE_NAME}_[PRPI-4872]_should_render_players_grid_correctly`))
          .misMatchPercentage,
      ).toEqual(0);
    });
  });

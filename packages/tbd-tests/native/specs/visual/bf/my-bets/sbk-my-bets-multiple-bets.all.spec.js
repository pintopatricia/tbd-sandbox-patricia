...
const { BottomBarSO, MyBetsScreenSO, SportsbookBetPanelSO, CardSO } = require("../../../../screen-objects");
...
describe("My Bets Page - SBK Multiple Bets", () => {
  describe("When the user opens My Bets page with one multiple bet with Acca Insurance", () => {
    beforeAll(async () => {
      ...
      await browser.waitUntilImageEquals(
        `${CARD_NAME}_[PRPI-4848]_the_multiple_bet_card_group_should_be_displayed_with_acca_insurance_signpost`,
      );
    });

    it("[PRPI-4848]_the_multiple_bet_card_group_should_be_displayed_with_acca_insurance_signpost", async () => {
      expect(
        (
    ...
    describe("and then clicks on card", () => {
      beforeAll(async () => {
        await browser.waitUntilClickableNative(cardSO.element, "The card button is not clickable");
        await cardSO.element.click();
        await browser.waitUntilDisplayed(cardSO.contentWrapper, "The card button is not expanded");
        await swipeUpElementFullscreen(cardSO.header);
        await browser.waitUntilStopsMoving(cardSO.element);
        await browser.waitUntilImageEquals(
          `${CARD_NAME}_[PRPI-4849]_the_bet_legs_information_and_the_scoreboards_should_be_displayed`,
        );
      });

      it("[PRPI-4849]_the_bet_legs_information_and_the_scoreboards_should_be_displayed", async () => {
        expect(
          (
            await browser.compareScreen(
              `${CARD_NAME}_[PRPI-4849]_the_bet_legs_information_and_the_scoreboards_should_be_displayed`,

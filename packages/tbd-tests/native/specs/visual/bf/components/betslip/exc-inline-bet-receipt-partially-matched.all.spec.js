...
describe("Inline Betslip - Partially matched receipt", () => {
  ...
  describe("when the user places a back bet and it is partially matched", () => {
    beforeAll(async () => {
      ...
      await browser.waitUntilImageEquals(`${MODULE_NAME}_[PRPI-10441]_should_display_the_partially_matched_receipt`);
    });

    it("[PRPI-10441]_should_display_the_partially_matched_receipt", async () => {
      expect(
        (await browser.compareScreen(`${MODULE_NAME}_[PRPI-10441]_should_display_the_partially_matched_receipt`))
          .misMatchPercentage,
      ).toEqual(0);
    });
  });

...
const doubleFreeBetsWalletsAlertSO = new AlertSO(multiplesCollapsableSO.element.$(`~${ALERT}`));
...
describe("SBK Generosity Wallet - Multiples - Confirm and Receipt", () => {
  beforeAll(async () => {
    ...
    await browser.waitUntilNotDisplayed(multiplesCollapsableSO.contentWrapper, "BET BUILDER section is not collapsed");
  ...
  describe("on multiples and place footer", () => {
    beforeAll(async () => {
      await browser.waitUntilDisplayed(doubleFreeBetsWalletsAlertSO.element, "DOUBLE Free Bets Alert is not displayed");
      await browser.waitUntilImageEquals(
        `${MODULE_NAME}_[PRPI-4956]_should_display_the_confirm_step_multiples_combinations_with_the_selected_wallets_amount_and_the_place_footer_total_selected_wallets_amount`,
      );
    });
    ...
    afterAll(async () => {
      await browser.waitUntilClickableNative(multiplesCollapsableSO.header);
      await multiplesCollapsableSO.header.click();
      await browser.waitUntilNotDisplayed(multiplesCollapsableSO.contentWrapper, "Multiples section is not collapsed");
    });

    it("[PRPI-4956]_should_display_the_confirm_step_multiples_combinations_with_the_selected_wallets_amount_and_the_place_footer_total_selected_wallets_amount", async () => {
      expect(
        (
  ...
  describe("on singles", () => {
    it("[PRPI-4957]_should_display_the_confirm_step_multiples_single_combinations_with_the_selected_wallets_amount", async () => {
      expect(
        (
          await browser.compareScreen(
            `${MODULE_NAME}_[PRPI-4957]_should_display_the_confirm_step_multiples_single_combinations_with_the_selected_wallets_amount`,
          )
        ).misMatchPercentage,
      ).toEqual(0);
    });
  ...
  describe("then the users places the bet", () => {
    beforeAll(async () => {
      ...
      await browser.waitUntilImageEquals(
        `${MODULE_NAME}_[PRPI-4958]_should_display_the_multiples_free_bets_wallets_generosity_alert_on_the_receipt`,
      );
    });

    it("[PRPI-4958]_should_display_the_multiples_free_bets_wallets_generosity_alert_on_the_receipt", async () => {
      expect(
        (
          await browser.compareScreen(
            `${MODULE_NAME}_[PRPI-4958]_should_display_the_multiples_free_bets_wallets_generosity_alert_on_the_receipt`,
    ...
    describe("then the user scrolls down", () => {
      ...
      it("[PRPI-4959]_should_display_the_singles_free_bets_wallets_generosity_alert_on_the_receipt", async () => {
        expect(
          (
            await browser.compareScreen(
              `${MODULE_NAME}_[PRPI-4959]_should_display_the_singles_free_bets_wallets_generosity_alert_on_the_receipt`,
            )
          ).misMatchPercentage,
        ).toEqual(0);
      });

...
const betControlsPO = new BetControlsSO();
const stakeInputFieldSO = new CurrencyNumberInputFieldSO(betControlsPO.currencyInput);
...
const placeNotification = new AlertSO(sportsbookReceiptPanelSO.element);
...
xdescribe("Price Boost - Receipt Panel", () => {
  beforeAll(async () => {
  ...
  describe("when users places a price boost multi bet", () => {
    beforeAll(async () => {
      ...
      await stakeInputFieldSO.numberField.setValue(20);
    ...
    it("[PRPI-1261]_should_render_pbm_receipt", async () => {
      expect(
        (await browser.compareScreen(`${MODULE_NAME}_[PRPI-1261]_should_render_pbm_receipt`)).misMatchPercentage,
      ).toEqual(0);
    });
  });
});

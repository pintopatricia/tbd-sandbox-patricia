...
describe("Inline Betslip - Edit unmatched bet", () => {
  ...
  describe("when the user places an unmatched bet and clicks Edit Bet", () => {
    beforeAll(async () => {
      ...
      await browser.waitUntilDisplayed(exchangeInlineEditPanelSO.element, "Edit panel wasn't displayed");

      await browser.waitUntilImageEquals(
        `${MODULE_NAME}_[PRPI-10440]_should_display_the_edit_panel_for_an_unmatched_bet`,
      );
    });

    it("[PRPI-10440]_should_display_the_edit_panel_for_an_unmatched_bet", async () => {
      expect(
        (await browser.compareScreen(`${MODULE_NAME}_[PRPI-10440]_should_display_the_edit_panel_for_an_unmatched_bet`))
          .misMatchPercentage,
      ).toEqual(0);
    });
  });

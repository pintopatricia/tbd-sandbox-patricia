...
describe("PopularBetBuilder", () => {
  describe("American Football Bet Builder", () => {
    beforeAll(async () => {
      ...
      await mockService.mockHttpRequest(getGenericLayout(BFF_AMERICAN_FOOTBALL_VIEW_MOCK));

      const HOME_VIEW_LINK = getStartViewLink(`american-football/s-${EVENT_TYPE_ID_AMERICAN_FOOTBALL}`);
      ...
      await browser.waitUntilImageEquals(
        `${CARD_NAME}_[PRPI-4526]_should_render_american_football_popular_bet_builder_card`,
      );
    });

    it("[PRPI-4526]_should_render_american_football_popular_bet_builder_card", async () => {
      expect(
        (

...
describe("PopularBetBuilder", () => {
  describe("Basketball Bet Builder", () => {
    beforeAll(async () => {
      ...
      await mockService.mockHttpRequest(getGenericLayout(BFF_BASKETBALL_VIEW_MOCK));

      const HOME_VIEW_LINK = getStartViewLink(`basketball/s-${EVENT_TYPE_ID_BASKETBALL}`);
      await mockService.mockHttpRequest(getHomeLayoutWithViewLink(HOME_VIEW_LINK));

      await startApp("home", { isViewLinkStartPage: !!HOME_VIEW_LINK });

      await browser.waitUntilEquals(sportsbookBetButtonSO.odd, "Add to Betslip at 81.81");
      await browser.waitUntilImageEquals(`${CARD_NAME}_[PRPI-4527]_should_render_basketball_popular_bet_builder_card`);
    });

    it("[PRPI-4527]_should_render_basketball_popular_bet_builder_card", async () => {
      expect(
        (await browser.compareScreen(`${CARD_NAME}_[PRPI-4527]_should_render_basketball_popular_bet_builder_card`))
          .misMatchPercentage,
      ).toBe(0);
    });
  });

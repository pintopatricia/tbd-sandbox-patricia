FROM docker.app.betfair/ppb/tbdn/rn-android-sbg:2.0.0 AS build-inhouse
...
COPY --from=build-inhouse /usr/app/apps/sbg/native/android/app/build/outputs/apk ./android/app/build/outputs/apk

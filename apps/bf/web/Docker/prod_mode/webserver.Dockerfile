...
COPY ./apps/${BRAND}/web/package.json ./apps/${BRAND}/web/
...
COPY packages/gql-doc-loader/ packages/gql-doc-loader/
COPY apps/${BRAND}/web/ apps/${BRAND}/web/
...
COPY apps/${BRAND}/reverse-proxy/prod/nginx.webserver.conf /etc/nginx/nginx.conf
COPY --from=build /usr/app/apps/${BRAND}/web/dist /usr/share/nginx/html

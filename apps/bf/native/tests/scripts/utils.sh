...
function request_simulators_ios {
    ...
    yarn run deviceProperties $platform $deviceName $platformVersion
}
...
function request_emulators_android {
    ...
    # update app.config.json file with deviceName and platformVersion
    yarn run deviceProperties $platform $deviceName
}
...
function request_environments {
    ...
    sed -i '' -e 's/}/, \"TBDN_DEFAULT_ENVIRONMENT\":\"'$environment'\"\}/g' ../app.config.json
}

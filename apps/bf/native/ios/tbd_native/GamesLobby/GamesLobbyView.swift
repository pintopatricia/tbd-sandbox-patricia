...
class GamesLobbyView: UIView {
  ...
  override init(frame: CGRect) {
    ...
    /// Subscribe to the goToHomeTab notification
    NotificationCenter.default.addObserver(forName: .goToHomeTab, object: nil, queue: nil) { object in
      Task { @MainActor in
        if let urlDeepLinking = object.object {
          let urlDeepLinkingObject = ["urlDeepLinking": urlDeepLinking]
          GamesLobbyEventEmitter.sendEvent(withName: "onGoToHomeTabScreen", body: urlDeepLinkingObject)
        }
      }
    }
  }
  ...
  @objc func openWebView(_ urlString: String, resolver: @escaping RCTPromiseResolveBlock) {
      ...
      DispatchQueue.main.async {
          ...
          if let parentVC = GamesLobbyView._parentVC, let webController = webController {
              parentVC.addChild(webController)
              parentVC.view.addSubview(webController.view)
              webController.view.fillSuperviewWidth()
              webController.view.fillSuperviewHeight()
              webController.didMove(toParent: parentVC)
              resolver(nil)
          } else {
              resolver(nil)
          }
  ...
  static var gamesFramework: GamesFramework? {
    didSet {
      ...
      framework.onViewControllerWillChange = { (viewController, segue, _) in
      ...
      framework.onGameDetailsEvent = { event, viewController, completion in
        ...
        case .show:
          if let currentViewController = _parentVC {
            ...
            GamesLobbyView.gameDetailsViewController = viewController
            GamesLobbyEventEmitter.sendEvent(withName: "onOpenGameDetailsScreen")
          }
        case .close, .viewWillDisappear:
          GamesLobbyView.gameDetailsViewController = nil
      ...
      framework.goToNativeAction = { webController in
      ...
      /// This callback is called to handle back action from WebViewController to Home Tab
      framework.goToHomeTabAction = { urlDeepLinking in
        
        NotificationCenter.default.post(name: .goToHomeTab, object: urlDeepLinking)
      }
    ...
    if let viewController = viewController {
      ...
      if !GamesLobbyView.gameContainerOpened, screen == .lobby {
          func launchGameIfNeeded() {
              guard !GamesLobbyView.gameContainerOpened, let gameLaunchInfo = GamesLobbyView.gameLaunchInfo, let gameUrl = URL(string: gameLaunchInfo.gameUrl) else { return }
              GamesLobbyView.gamesFramework?.launchGame(
                  withId: gameLaunchInfo.gameId,
                  isFromCPP: gameLaunchInfo.isFromCpp,
                  isFromWeb: true,
                  from: viewController,
                  gameUrl: gameUrl
              )
  ...
  static private func onOpenGameCollectionScreen(with viewController: UIViewController) {
    ...
    GamesLobbyEventEmitter.sendEvent(withName: "onOpenGameCollectionScreen")
  }
}

...
##############################################
echo
echo "You're about to setup everything to build Android..."
echo
...
if [ -z "${MODE}" ]; then
  ...
  echo "ℹ️  Please provide a MODE environment variable with a valid value, such as ${GREEN}internal${NC} (e.g.: MODE=internal yarn android)."
...
if [ -z "${ENV}" ]; then
  ...
  echo "ℹ️  Please provide a ENV environment variable with a valid value, such as ${GREEN}qa${NC} (e.g: ENV=qa yarn android)."
...
##############################################
echo
echo "You're about to build an Android app with the following configuration..."
echo
echo "🚀  ${BLUE}${APP_BRAND}${NC}(${MODE}) ▶▶▶ ${PURPLE}${ENV}${NC}"
echo "   Variant: ${GREEN}${VARIANT}${NC}"
echo "   Suffix: ${GREEN}${SUFFIX}${NC}"
echo

# Print necessary setup to run Android Release builds locally
if [ "${MODE}" == "production" ]; then
  source ./scripts/get-release-env-vars.sh
fi

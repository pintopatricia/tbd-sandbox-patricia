...
import { navigate, navigationRef } from "@ppb/tbd-router/native";
...
import { ProductsOption, UserDetails } from "@ppb/tbd-store";
...
const FeedbackWrapper: FunctionComponent<{ onFeedbackTap: () => void }> = ({ onFeedbackTap }) => {
  const getProductPreference = useMemo(() => createProductPreferenceWithProductSwitcherSelector(), []);
  const { currentProduct, isErrorView, isSbkThrottle, isExcThrottle, loggedIn, currentView } = useSelector(
    (state: ApplicationState) => ({
      currentProduct: getProductPreference(state.entities.preferences),
      isErrorView: !!(state.layouts.views.error && Object.keys(state.layouts.views.error).length > 0),
      isSbkThrottle: !!state.entities.throttles.FEEDBACK_BUTTON_NATIVE?.isActive,
      isExcThrottle: !!state.entities.throttles.EXC_FEEDBACK_BUTTON?.isActive,
      loggedIn: state.entities.userdetails.loggedIn,
      currentView: state.router.currentView,
    }),
    shallowEqual,
  );

  const shouldShowFeedbackButton = useMemo(() => {
    const isSbk = isSbkThrottle && currentProduct === ProductsOption.sportsbook;
    const isExc = isExcThrottle && loggedIn && currentProduct === ProductsOption.exchange;
    const isGamingView = currentView?.includes(EntityType.GameView) || currentView?.includes(EntityType.GamingView);
    return !isErrorView && (isSbk || (isExc && !isGamingView));
  }, [currentProduct, isErrorView, isSbkThrottle, isExcThrottle, loggedIn, currentView]);

  if (!shouldShowFeedbackButton) return null;

  return (
    <View style={styles.feedbackButton}>
      <Feedback onFeedbackTap={onFeedbackTap} />
    </View>
  );
};
...
function App({ store }: { store: IModuleStore<ApplicationState> }): JSX.Element | null {
  ...
  const handleFeedbackTap = useCallback(() => {
    ...
    const { accountId } = state.entities.userdetails as UserDetails;

    if (currentProduct === ProductsOption.exchange) {
      const baseUrl = appConfig.EXC_FEEDBACK_URL;
      const viewUrl = accountId ? `${baseUrl}?param=${accountId}` : baseUrl;
      navigate({
        viewUrl,
                        ...
                        <OffsetProvider>
                          <ScrollIdleProvider>
                            <RefreshEnabledProvider>
                              <FeedbackWrapper onFeedbackTap={handleFeedbackTap} />
                              {showFeedbackModal && (

...
COPY ./apps/ps/web/package.json ./apps/ps/web/

# install deps and
RUN yarn workspaces focus @ppb/tbdps-webapp
...
COPY apps/ps/reverse-proxy/prod/nginx.webserver.conf /etc/nginx/nginx.conf
COPY --from=build /usr/app/apps/ps/web/dist /usr/share/nginx/html

%define name    tbdps-mobile-site
%define version 1.0.0
%define release %{_buildnumber}

Summary: PokerStars TBD
Name: %{name}
Version: %{version}
Release: %{release}
Vendor: Betfair
...
Group: Software
Provides: tbdps-mobile-site
Requires: bf-fabric = 28.0.0-307
Requires: bf-tbd-http-bff-gql-v11 = 11.99.1-608
# NOTE: If query was updated, also bump http-webserver
Requires: bf-tbd-http-webserver-v1 = 1.4.0-1347
BuildArch: x86_64
...
rm -rf %{buildroot}
# Create /var/app/tbdps-mobile-site folders on build_root and copy app files to it
mkdir -p %{buildroot}%{_localstatedir}/app/%{name}
cp -r %{_sourcedir}/* %{buildroot}%{_localstatedir}/app/%{name}
# Create default folder
%{__mkdir} -p %{buildroot}%{_sysconfdir}/%{name}
# Create init script symlink from /etc/init.d/bf-fabric to /etc/init.d/tbdps-mobile-site
%{__mkdir} -p %{buildroot}%{_sysconfdir}/init.d
ln -s %{_sysconfdir}/init.d/bf-fabric %{buildroot}%{_sysconfdir}/init.d/%{name}
